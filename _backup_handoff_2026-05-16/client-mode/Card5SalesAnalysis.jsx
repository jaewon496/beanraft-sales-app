...
const Card5SalesAnalysis = ({ card, kosisBoxData = null }) => {
  ...
  return (
    ...
    >
          ...
          {showSection1 && (
            ...
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={TOK.onSurfaceVariant} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ cursor: 'help' }} aria-hidden="true">
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
                {usageCnt && (
                  <div>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={TOK.primary} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
                ...
                {unitPrice && (
                  <div>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={TOK.secondary} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
      ...
      {/* ── 한국은행 권역 소비심리 박스 + 12개월 추이 라인 ── */}
      {kosisBoxData?.consumerSentiment && (() => {
        const cs = kosisBoxData.consumerSentiment;
        const score = Number(cs.value) || 0;
        const isUp = score >= 100;
        const tone = isUp ? '#10B981' : '#EF4444';
        const toneBg = isUp ? 'rgba(16, 185, 129, 0.08)' : 'rgba(239, 68, 68, 0.08)';
        const toneLabel = isUp ? '소비 회복세' : '소비 위축';
        const periodTxt = cs.period || '';
        // 202403 → 2024년 3월
        const periodLabel = /^\d{6}$/.test(periodTxt)
          ? `${periodTxt.slice(0, 4)}년 ${parseInt(periodTxt.slice(4), 10)}월`
          : periodTxt;
        const regionLabel = cs.region || kosisBoxData.sido || '전국';

        // 12개월 시계열
        const series12 = kosisBoxData?.consumerSentimentSeries?.series || [];
        const hasSeries = Array.isArray(series12) && series12.length > 1;
        ...
        return (
          <div style={{
            ...
            <div style={{ fontSize: 13, fontWeight: 700, color: TOK.white, letterSpacing: '-0.01em' }}>
              이번 달 권역 소비심리
            </div>
            <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, marginTop: 4 }}>
              <span style={{ fontSize: 32, fontWeight: 700, color: TOK.white, fontFamily: TOK.fontH1, lineHeight: 1.1 }}>
                {score.toFixed(1)}
              </span>
              <span style={{ fontSize: 13, color: TOK.onSurfaceVariant }}>점 / 기준 100점</span>
              <span style={{
                marginLeft: 'auto',
                fontSize: 12, fontWeight: 700, color: tone,
                padding: '4px 10px', borderRadius: 999,
                background: tone === '#10B981' ? 'rgba(16, 185, 129, 0.15)' : 'rgba(239, 68, 68, 0.15)',
              }}>
                {toneLabel}
              </span>
            </div>
            ...
            {/* 12개월 추이 라인 (있을 때만) */}
            {hasSeries && (() => {
              const vals = series12.map(s => Number(s.value) || 0);
              const periods = series12.map(s => s.period || '');
              const w = 320, h = 70;
              const padX = 6, padY = 10;
              // 100 기준선 포함하여 범위 계산
              const allVals = [...vals, 100];
              const maxV = Math.max(...allVals);
              const minV = Math.min(...allVals);
              const range = (maxV - minV) || 1;
              const stepX = (w - padX * 2) / (vals.length - 1);
              const yOf = (v) => h - padY - ((v - minV) / range) * (h - padY * 2);
              const baselineY = yOf(100);
              // 100 위/아래로 색상 구분: 100 위 = 초록 영역, 아래 = 빨강 영역
              // 두 개의 면적 path 생성
              const linePts = vals.map((v, i) => `${(padX + i * stepX).toFixed(1)},${yOf(v).toFixed(1)}`).join(' ');
              ...
              return (
                <div style={{ marginTop: 12, padding: '10px 0 4px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 4 }}>
                    <span style={{ fontSize: 11, color: TOK.onSurfaceVariant }}>최근 12개월 추이</span>
                    {change !== 0 && (
                  ...
                  <svg width="100%" height={h} viewBox={`0 0 ${w} ${h}`} preserveAspectRatio="none" style={{ overflow: 'visible' }}>
                    <defs>
                      <clipPath id="cs-above-100">
                        <rect x="0" y="0" width={w} height={baselineY} />
                      </clipPath>
                      <clipPath id="cs-below-100">
                        <rect x="0" y={baselineY} width={w} height={h - baselineY} />
                      </clipPath>
                    </defs>
                    {/* 100 기준선 */}
                    <line x1={padX} y1={baselineY} x2={w - padX} y2={baselineY} stroke="rgba(255,255,255,0.25)" strokeWidth="0.5" strokeDasharray="2,2" />
                    <text x={w - padX} y={baselineY - 3} textAnchor="end" fill="rgba(255,255,255,0.45)" fontSize="9">기준 100</text>
                    {/* 100 위 부분 - 초록 라인 */}
                    <polyline points={linePts} fill="none" stroke="#10B981" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" clipPath="url(#cs-above-100)" />
                    {/* 100 아래 부분 - 빨강 라인 */}
                    <polyline points={linePts} fill="none" stroke="#EF4444" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" clipPath="url(#cs-below-100)" />
                    {/* 마지막 점 */}
                    {(() => {
                      const lx = padX + (vals.length - 1) * stepX;
                      const ly = yOf(last);
                      const dotColor = last >= 100 ? '#10B981' : '#EF4444';
                      return <circle cx={lx} cy={ly} r="2.5" fill={dotColor} stroke="#fff" strokeWidth="0.5" />;
                    })()}
                  </svg>
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 9, color: TOK.onSurfaceVariant, marginTop: 2 }}>
                    <span>{(periods[0] || '').replace(/(\d{4})(\d{2})/, '$1·$2월')}</span>
                    <span>{(periods[periods.length - 1] || '').replace(/(\d{4})(\d{2})/, '$1·$2월')}</span>
                  </div>
                </div>
              );
            })()}

            <div style={{ fontSize: 11, color: TOK.onSurfaceVariant, marginTop: 8, textAlign: 'right' }}>
              한국은행 월별 소비자동향조사 · {regionLabel} · {periodLabel}
            </div>
          </div>
        );
      })()}
