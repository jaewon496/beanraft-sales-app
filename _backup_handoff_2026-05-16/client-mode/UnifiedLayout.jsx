...
// ─── 숫자를 한국어 발음으로 변환 (TTS 자연스러운 발음용) ───
// 예: "352개" → "삼백오십이 개", "9,121만원" → "구천백이십일만 원", "57.9만명" → "오십칠 점 구만 명", "60%" → "육십 퍼센트"
function _korFromInt(n) {
  if (n === 0) return '영';
  ...
  let result = '';
  groups.forEach((g, gi) => {
    const bi = groups.length - 1 - gi;
    let part = '';
    const gd = g.split('').map(Number);
    gd.forEach((d, di) => {
      const sui = gd.length - 1 - di;
      if (d === 0) return;
      if (d === 1 && sui > 0) part += S[sui];
      else part += D[d] + S[sui];
    });
    if (part) result += part + B[bi];
  });
  return n < 0 ? '마이너스 ' + result : result;
}
function numbersToKorean(text) {
  ...
  // 부호(+/-) + 숫자(쉼표 포함) + 소수점 + 단위(만원/만명/만/원/명/개/곳/위/건/년/월/일/시/분/점/%/km/m)
  const re = /([+\-]?)([\d][\d,]*(?:\.\d+)?)(\s*)(만\s*원|만\s*명|만|원|명|개|곳|위|건|년|월|일|시|분|점|%|km|m)?/g;
  return text.replace(re, (match, sign, numStr, sp, unit) => {
    const cleaned = numStr.replace(/,/g, '');
    const negative = sign === '-';
    const positive = sign === '+';
    const absStr = cleaned;
    const [intPart, decPart] = absStr.split('.');
    const intNum = parseInt(intPart || '0', 10);
    if (isNaN(intNum)) return match;
    let kor = (negative ? '마이너스 ' : positive ? '플러스 ' : '') + _korFromInt(intNum);
    if (decPart) kor += ' 점 ' + decPart.split('').map(d => D[Number(d)] || '영').join(' ');
    if (unit) {
      const trimmed = unit.replace(/\s+/g, '');
      const u = unitMap[trimmed] || (' ' + unit);
      kor += u;
    }
    return kor;
  });
...
// ─── 초기 투자비 / 월 고정비 풀너비 분해 박스 (Card 8 본문) ───
const InvestmentBreakdownBox = ({ data }) => {
  if (!data) return null;
  const items = data.items || [];
  // 항목 분류: 보증금/권리금/인테리어/설비 = 1회성, 월임대 = 월 고정
  const oneTimeItems = items.filter(it => /보증금|권리금|인테리어|설비|초기/.test(it.label || ''));
  const monthlyItems = items.filter(it => /월임대|월세|월\s*임대|월정/.test(it.label || ''));
  const oneTimeTotal = oneTimeItems.reduce((s, it) => s + (it.value || 0), 0);
  const monthlyTotal = monthlyItems.reduce((s, it) => s + (it.value || 0), 0);
  const fmtWon = (n) => {
  ...
  // 1회성 항목별 비중 막대 (스택)
  const oneTimeBars = oneTimeItems.map((it, i) => {
    const pct = oneTimeTotal > 0 ? (it.value / oneTimeTotal) * 100 : 0;
    const colors = ['#3182F6', '#8B5CF6', '#F59E0B', '#10B981', '#EC4899'];
    return { ...it, pct, color: colors[i % colors.length] };
  });
  return (
    <div style={{ marginTop: 18 }}>
      <div style={{ fontSize: 13, fontWeight: 700, color: COLORS.white, marginBottom: 12 }}>창업 비용 구조</div>
      <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr', gap: 14 }}>
        {/* 좌측: 1회성 초기 투자비 (큰 박스 + 항목별 막대) */}
        <div style={{
          background: 'linear-gradient(135deg, rgba(49,130,246,0.12), rgba(139,92,246,0.08))',
          border: '1px solid rgba(49,130,246,0.3)', borderRadius: 12, padding: '18px 20px',
        }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 6 }}>
            <span style={{ fontSize: 12, color: '#93C5FD', fontWeight: 700, letterSpacing: '0.04em' }}>1회성 초기 투자비</span>
            <span style={{ fontSize: 11, color: COLORS.textMuted }}>한 번 들어가는 금액</span>
          </div>
          <div style={{ fontSize: 30, fontWeight: 800, color: COLORS.white, letterSpacing: '-0.03em', marginBottom: 14 }}>
            {fmtWon(oneTimeTotal)}<span style={{ fontSize: 14, color: '#93C5FD', marginLeft: 6 }}>원</span>
          </div>
          {/* 스택 막대 */}
          {oneTimeTotal > 0 && (
            <div style={{ display: 'flex', height: 12, borderRadius: 6, overflow: 'hidden', marginBottom: 12 }}>
              {oneTimeBars.map((b, i) => (
                <div key={i} title={`${b.label} ${fmtWon(b.value)}`} style={{ width: b.pct + '%', background: b.color, transition: 'width 0.3s' }} />
              ))}
            </div>
          )}
          {/* 항목별 분해 */}
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 8 }}>
            {oneTimeBars.map((b, i) => (
              <div key={i} style={{
                ...
                <span style={{ fontSize: 11, color: COLORS.textSecondary }}>{b.label}</span>
                <div style={{ textAlign: 'right' }}>
                  <div style={{ fontSize: 13, color: COLORS.white, fontWeight: 700 }}>{fmtWon(b.value)}</div>
                  <div style={{ fontSize: 9, color: COLORS.textMuted }}>{Math.round(b.pct)}%</div>
                </div>
              </div>
            ))}
          </div>
        ...
        <div style={{
          ...
          border: '1px solid rgba(245,158,11,0.3)', borderRadius: 12, padding: '18px 20px',
          display: 'flex', flexDirection: 'column',
        }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 6 }}>
            <span style={{ fontSize: 12, color: '#FCD34D', fontWeight: 700, letterSpacing: '0.04em' }}>월 고정비</span>
            <span style={{ fontSize: 11, color: COLORS.textMuted }}>매달 반복 지출</span>
          </div>
          <div style={{ fontSize: 30, fontWeight: 800, color: COLORS.white, letterSpacing: '-0.03em', marginBottom: 14 }}>
            {fmtWon(monthlyTotal)}<span style={{ fontSize: 14, color: '#FCD34D', marginLeft: 6 }}>원/월</span>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {monthlyItems.map((it, i) => (
              <div key={i} style={{
                ...
                <span style={{ fontSize: 12, color: COLORS.textSecondary }}>{it.label}</span>
                <span style={{ fontSize: 14, color: COLORS.white, fontWeight: 700 }}>{fmtWon(it.value)}</span>
              </div>
            ))}
          </div>
          {/* 연간 부담 환산 */}
          {monthlyTotal > 0 && (
            <div style={{
              marginTop: 'auto', paddingTop: 12, fontSize: 11, color: COLORS.textMuted,
              borderTop: '1px dashed rgba(255,255,255,0.1)',
            }}>
              연간 부담 약 <span style={{ color: '#FCD34D', fontWeight: 700 }}>{fmtWon(monthlyTotal * 12)}원</span>
            </div>
          )}
        </div>
      </div>
...
// ─── KOSIS 카페 풀너비 박스 (Card 8 본문에 표시) ───
const KosisCafeFullBox = ({ data }) => {
  const [simPyeong, setSimPyeong] = useState(20);
  if (!data) return null;
  // KOSIS 카페 데이터가 없어도 권리금만 있으면 권리금만 표시
  const k = data.kosisCafe || null;
  const hasKosis = !!(k && (k.interiorAvg || k.salesAvg));
  const hasPremium = !!data.premium;
  if (!hasKosis && !hasPremium) return null;
  const dist = (hasKosis && k?.interiorDistribution?.distribution) || [];
  const maxPct = dist.length ? Math.max(...dist.map(d => d.normalizedPct)) : 0;
  const simRange = pyeongToRangeIdx(simPyeong);
  const simBreakdown = pyeongToInteriorRange(simPyeong);
    ...
    <div style={{ marginTop: 18 }}>
      ...
      {hasKosis && (
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, marginBottom: 12 }}>
          <span style={{ fontSize: 13, fontWeight: 700, color: '#FCD34D' }}>전국 카페 평균</span>
          <span style={{ fontSize: 11, color: COLORS.textMuted }}>통계청 외식업체경영실태조사 {k.year}</span>
        </div>
      )}

      {/* 6개 지표 그리드 (값 있는 항목만 표시) */}
      {hasKosis && (() => {
        const items = [
          { label: '인테리어비', value: k.interiorAvg ? `${k.interiorAvg.toLocaleString()}만` : null },
          { label: '총 투자비', value: k.startupInvestAvg ? `${k.startupInvestAvg.toLocaleString()}만` : null },
          { label: '평균 평수', value: k.avgAreaPyeong ? `${k.avgAreaPyeong}평` : null },
          { label: '월 매출', value: k.salesAvg ? `${k.salesAvg.toLocaleString()}만` : null },
          { label: '객단가', value: k.unitPriceAvg ? `${k.unitPriceAvg.toLocaleString()}원` : null },
          { label: '평당 인테리어', value: k.interiorPerPyeong ? `${k.interiorPerPyeong}만` : null, accent: true },
        ].filter(it => it.value);
        ...
        return (
          <div style={{ display: 'grid', gridTemplateColumns: `repeat(${cols}, 1fr)`, gap: 10, marginBottom: 16 }}>
            {items.map((item, i) => (
              <div key={i} style={{
                background: item.accent ? 'rgba(245,158,11,0.12)' : 'rgba(255,255,255,0.04)',
                border: item.accent ? '1px solid rgba(245,158,11,0.4)' : '1px solid rgba(255,255,255,0.08)',
                borderRadius: 10, padding: '14px 12px', textAlign: 'center',
              }}>
                <div style={{ fontSize: 11, color: item.accent ? '#FCD34D' : COLORS.textMuted, marginBottom: 6, fontWeight: 600 }}>{item.label}</div>
                <div style={{ fontSize: 18, color: COLORS.white, fontWeight: 700, letterSpacing: '-0.02em' }}>{item.value}</div>
              </div>
            ))}
          </div>
        );
      ...
      {/* 권리금 (중소벤처기업부 상가건물임대차실태조사 2023, 시도별 평균) - 별도 출처라 자체 헤더 */}
      {data?.premium && (
        <div style={{ marginTop: hasKosis ? 18 : 0, marginBottom: 16 }}>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, marginBottom: 8 }}>
            <span style={{ fontSize: 13, fontWeight: 700, color: '#FCD34D' }}>지역 권리금</span>
            <span style={{ fontSize: 11, color: COLORS.textMuted }}>중소벤처기업부 상가건물임대차실태조사 {data.premium.year}</span>
          </div>
          <div style={{
            display: 'flex', alignItems: 'center', justifyContent: 'space-between',
            ...
            <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, flexWrap: 'wrap' }}>
              <span style={{ fontSize: 18, color: COLORS.white, fontWeight: 800, letterSpacing: '-0.02em' }}>
                {data.premium.sidoAvg
                  ? `${data.premium.sidoAvg.toLocaleString()}만원`
                  : `${data.premium.nationalAvg.toLocaleString()}만원`}
              </span>
              {data.premium.sidoKey ? (
                <span style={{ fontSize: 11, color: COLORS.textMuted }}>
                  {data.premium.sidoKey} 전체 업종 평균 · 받을 예정 {data.premium.sidoReceivePct}%
                </span>
              ) : (
                <span style={{ fontSize: 11, color: COLORS.textMuted }}>
                  전국 전체 업종 평균 · 받을 예정 {data.premium.nationalReceivePct}%
                </span>
              )}
            </div>
            <span style={{ fontSize: 10, color: COLORS.textMuted, whiteSpace: 'nowrap' }}>
              음식점업 전국 {data.premium.foodAvg.toLocaleString()}만
            </span>
          </div>
        </div>
      )}

      {/* 좌우 분할: 좌측 분포 막대 + 우측 평수 시뮬레이터 */}
      {hasKosis && (
      <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: 14 }}>
        {/* 좌측: 비용 분포 막대 */}
        {dist.length > 0 && (
          <div style={{
            background: 'rgba(0,0,0,0.3)', borderRadius: 12, padding: '16px 18px',
            border: '1px solid rgba(245,158,11,0.2)',
          }}>
            <div style={{ fontSize: 13, color: '#FCD34D', fontWeight: 700, marginBottom: 4 }}>
              실제 카페 인테리어 비용 분포
            </div>
            <div style={{ fontSize: 11, color: COLORS.textMuted, marginBottom: 14 }}>
              응답자 {Math.round(k.interiorDistribution.totalAnsweredPct)}% 기준 (모름 응답 제외)
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {dist.map((d, i) => {
                const isMax = d.normalizedPct === maxPct;
                const isHighlight = i === simRange;
                return (
                  <div key={i} style={{ display: 'grid', gridTemplateColumns: '110px 1fr 90px', alignItems: 'center', gap: 10 }}>
                    <div style={{ fontSize: 11, color: isHighlight ? '#FCD34D' : (isMax ? COLORS.white : COLORS.textMuted), fontWeight: isHighlight ? 700 : 500 }}>{d.range}</div>
                    <div style={{ height: 26, background: 'rgba(255,255,255,0.04)', borderRadius: 6, overflow: 'hidden', position: 'relative' }}>
                      <div style={{
                        width: (d.normalizedPct / maxPct * 100) + '%', height: '100%',
                        background: isHighlight
                          ? 'linear-gradient(90deg, #F59E0B, #FBBF24)'
                          : (isMax ? 'linear-gradient(90deg, rgba(245,158,11,0.55), rgba(251,191,36,0.55))' : 'rgba(255,255,255,0.18)'),
                        transition: 'all 0.3s ease',
                      }} />
                      <div style={{ position: 'absolute', left: 10, top: 5, fontSize: 12, color: COLORS.white, fontWeight: 700, textShadow: '0 0 4px rgba(0,0,0,0.85)' }}>{d.normalizedPct}%</div>
                    </div>
                    <div style={{ fontSize: 11, color: isHighlight ? '#FCD34D' : COLORS.textMuted, textAlign: 'right', fontWeight: isHighlight ? 700 : 500 }}>{d.tier}</div>
                  </div>
                );
              })}
            </div>
            <div style={{ fontSize: 10, color: COLORS.textMuted, marginTop: 14, lineHeight: 1.5, padding: '8px 10px', background: 'rgba(255,255,255,0.02)', borderRadius: 6 }}>
              ※ 응답자 기준 재계산. 작은 매장(셀프 시공 비중 높음)이 평균을 끌어내릴 수 있음
            </div>
          </div>
        )}
        ...
        <div style={{
          ...
          <div style={{ fontSize: 11, color: COLORS.textMuted, marginBottom: 14 }}>
            슬라이더로 평수 조절 → 예상 비용 자동 계산
          </div>

          <div style={{ textAlign: 'center', padding: '12px 0', marginBottom: 12, background: 'rgba(245,158,11,0.08)', borderRadius: 10 }}>
            <div style={{ fontSize: 36, color: COLORS.white, fontWeight: 800, letterSpacing: '-0.04em', lineHeight: 1 }}>{simPyeong}<span style={{ fontSize: 16, color: '#FCD34D', marginLeft: 4 }}>평</span></div>
            <div style={{ fontSize: 11, color: COLORS.textMuted, marginTop: 6 }}>{simBreakdown.tier}</div>
          </div>

          <input
            type="range" min="5" max="60" step="1" value={simPyeong}
            onChange={(e) => setSimPyeong(parseInt(e.target.value))}
            style={{ width: '100%', accentColor: '#F59E0B', marginBottom: 6 }}
          />
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 9, color: COLORS.textMuted, marginBottom: 14 }}>
            <span>5평</span><span>20평</span><span>40평</span><span>60평</span>
          </div>

          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginTop: 'auto' }}>
            <div style={{ background: 'rgba(255,255,255,0.05)', padding: '12px 12px', borderRadius: 8, textAlign: 'center' }}>
              <div style={{ fontSize: 10, color: COLORS.textMuted, marginBottom: 6 }}>예상 인테리어비</div>
              <div style={{ fontSize: 16, color: COLORS.white, fontWeight: 700 }}>{simBreakdown.totalMin.toLocaleString()}<span style={{ fontSize: 11, color: COLORS.textMuted }}>~</span>{simBreakdown.totalMax.toLocaleString()}<span style={{ fontSize: 11, color: COLORS.textMuted, marginLeft: 2 }}>만</span></div>
            </div>
            <div style={{ background: 'rgba(255,255,255,0.05)', padding: '12px 12px', borderRadius: 8, textAlign: 'center' }}>
              <div style={{ fontSize: 10, color: COLORS.textMuted, marginBottom: 6 }}>평당 단가</div>
              <div style={{ fontSize: 16, color: COLORS.white, fontWeight: 700 }}>{simBreakdown.minPerPy}<span style={{ fontSize: 11, color: COLORS.textMuted }}>~</span>{simBreakdown.maxPerPy}<span style={{ fontSize: 11, color: COLORS.textMuted, marginLeft: 2 }}>만</span></div>
            </div>
          </div>
          <div style={{ fontSize: 10, color: COLORS.textMuted, marginTop: 12, lineHeight: 1.5, padding: '8px 10px', background: 'rgba(255,255,255,0.02)', borderRadius: 6 }}>
            ※ 일반 카페(개인) 시장가 기준. 컨셉/마감재에 따라 변동
          </div>
        </div>
      </div>
      )}
...
// ─── KOSIS 인테리어비 시뮬레이터 helper ───
// 평수 → 일반 카페(개인) 시장가 기준 인테리어비 구간
function pyeongToInteriorRange(pyeong) {
  if (pyeong < 9) return { tier: '소형 (셀프 비중 높음)', minPerPy: 100, maxPerPy: 200, totalMin: pyeong * 100, totalMax: pyeong * 200 };
  if (pyeong < 15) return { tier: '중소형 (저예산~일반)', minPerPy: 150, maxPerPy: 250, totalMin: pyeong * 150, totalMax: pyeong * 250 };
  if (pyeong < 30) return { tier: '중형 (일반~컨셉)', minPerPy: 200, maxPerPy: 300, totalMin: pyeong * 200, totalMax: pyeong * 300 };
  if (pyeong < 60) return { tier: '대형 (컨셉~고급)', minPerPy: 250, maxPerPy: 350, totalMin: pyeong * 250, totalMax: pyeong * 350 };
  return { tier: '초대형 (규모 효과로 평당 절감)', minPerPy: 200, maxPerPy: 300, totalMin: pyeong * 200, totalMax: pyeong * 300 };
}
// 평수 → KOSIS 비용 분포의 어느 구간 인덱스에 속하는지 (시뮬레이터 강조용)
function pyeongToRangeIdx(pyeong) {
  const range = pyeongToInteriorRange(pyeong);
  const midTotal = (range.totalMin + range.totalMax) / 2;
  // 0: 1천 미만, 1: 1~2천, 2: 2~5천, 3: 5천~1억, 4: 1억+
  if (midTotal < 1000) return 0;
  if (midTotal < 2000) return 1;
  if (midTotal < 5000) return 2;
  if (midTotal < 10000) return 3;
  return 4;
}

// ─── Card 7: Price Cards (임대/창업 정보) ───
const ChartPriceCards = ({ data }) => {
  // 평수 시뮬레이터 (KOSIS 분포에서 사용자가 자기 카페 위치 보기용)
  const [simPyeong, setSimPyeong] = useState(20);
  const hasData = data && Array.isArray(data.items) && data.items.length > 0;
  ...
  // 사용자가 한눈에 알고 싶은 핵심: 보증금 + 월임대
  const depositItem = items.find(it => /보증금/.test(it.label || ''));
  const rentItem = items.find(it => /월임대|월세|월\s*임대/.test(it.label || ''));

  return (
    <div style={{ padding: '24px 24px 20px', display: 'flex', flexDirection: 'column', height: '100%', justifyContent: 'center' }}>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
        ...
        <div style={{
          ...
          <div style={{ fontSize: 12, color: COLORS.textMuted, marginBottom: 10, fontWeight: 500 }}>보증금</div>
          <div style={{ fontSize: 28, fontWeight: 800, color: COLORS.white, letterSpacing: '-0.03em' }}>
            {fmtWon(depositItem?.value || 0)}<span style={{ fontSize: 13, color: COLORS.textMuted, marginLeft: 4 }}>원</span>
          </div>
        </div>
        ...
        <div style={{
          ...
          <div style={{ fontSize: 12, color: COLORS.textMuted, marginBottom: 10, fontWeight: 500 }}>월임대</div>
          <div style={{ fontSize: 28, fontWeight: 800, color: COLORS.white, letterSpacing: '-0.03em' }}>
            {fmtWon(rentItem?.value || 0)}<span style={{ fontSize: 13, color: COLORS.textMuted, marginLeft: 4 }}>원/월</span>
          </div>
        </div>
      </div>

      {/* KOSIS 카페 전국 평균은 본문(KosisCafeFullBox)으로 이동 - 풀너비 표시 */}

      {/* [Firebase 빈크래프트 수집기] 지역별 월세/보증금 보강 섹션 */}
      {data?.rentBase && !data.rentBase.isEstimate && data.rentBase.totalArticles > 0 && (
        <div style={{ marginTop: 14 }}>
          {/* 검색 동/시군구 vs 주변 평균 vs 중위값 비교 */}
          <div style={{
            display: 'grid',
            gridTemplateColumns: data.rentBase.isSigunguLevel ? '1fr' : 'repeat(3, 1fr)',
            gap: 8,
            marginBottom: 10,
          }}>
            {/* 시군구 단위인 경우: 한 박스 */}
            {data.rentBase.isSigunguLevel && data.rentBase.primaryMonthly > 0 ? (
              <div style={{ background: 'rgba(255,255,255,0.04)', borderRadius: 10, padding: '14px 14px', border: '1px solid rgba(255,255,255,0.08)' }}>
                <div style={{ fontSize: 12, color: COLORS.textMuted, marginBottom: 8 }}>{data.rentBase.primaryDong} 평균 (시군구 단위)</div>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', gap: 16 }}>
                  <div>
                    <div style={{ fontSize: 11, color: COLORS.textMuted }}>월세</div>
                    <div style={{ fontSize: 16, color: COLORS.white, fontWeight: 700 }}>{fmtWon(data.rentBase.primaryMonthly)}</div>
                  </div>
                  <div>
                    <div style={{ fontSize: 11, color: COLORS.textMuted }}>보증금</div>
                    <div style={{ fontSize: 16, color: COLORS.white, fontWeight: 700 }}>{fmtWon(data.rentBase.primaryDeposit)}</div>
                  </div>
                  {data.rentBase.perPyeong > 0 && (
                    <div>
                      <div style={{ fontSize: 11, color: COLORS.textMuted }}>평당 월세</div>
                      <div style={{ fontSize: 16, color: COLORS.white, fontWeight: 700 }}>{fmtWon(data.rentBase.perPyeong)}</div>
                    </div>
                  )}
                </div>
              </div>
              ...
              <>
                {/* 검색 동 직접 */}
                {data.rentBase.primaryMonthly > 0 && (
                  <div style={{ background: 'rgba(255,255,255,0.04)', borderRadius: 10, padding: '12px 10px', border: '1px solid rgba(255,255,255,0.08)' }}>
                    <div style={{ fontSize: 11, color: COLORS.textMuted, marginBottom: 6 }}>{(data.rentBase.primaryDong || '검색 동') + ' 직접'}</div>
                    <div style={{ fontSize: 14, color: COLORS.white, fontWeight: 700, lineHeight: 1.3 }}>월 {fmtWon(data.rentBase.primaryMonthly)}</div>
                    <div style={{ fontSize: 11, color: COLORS.textMuted, marginTop: 2 }}>보증금 {fmtWon(data.rentBase.primaryDeposit)}</div>
                  </div>
                )}
                {/* 주변 평균 (가중평균) */}
                {data.rentBase.finalMonthly > 0 && (
                  <div style={{ background: 'rgba(255,255,255,0.04)', borderRadius: 10, padding: '12px 10px', border: '1px solid rgba(255,255,255,0.08)' }}>
                    <div style={{ fontSize: 11, color: COLORS.textMuted, marginBottom: 6 }}>주변 평균</div>
                    <div style={{ fontSize: 14, color: COLORS.white, fontWeight: 700, lineHeight: 1.3 }}>월 {fmtWon(data.rentBase.finalMonthly)}</div>
                    <div style={{ fontSize: 11, color: COLORS.textMuted, marginTop: 2 }}>보증금 {fmtWon(data.rentBase.finalDeposit)}</div>
                  </div>
                )}
                {/* 중위값 */}
                {data.rentBase.medianMonthly > 0 && (
                  <div style={{ background: 'rgba(255,255,255,0.04)', borderRadius: 10, padding: '12px 10px', border: '1px solid rgba(255,255,255,0.08)' }}>
                    <div style={{ fontSize: 11, color: COLORS.textMuted, marginBottom: 6 }}>중간값</div>
                    <div style={{ fontSize: 14, color: COLORS.white, fontWeight: 700, lineHeight: 1.3 }}>월 {fmtWon(data.rentBase.medianMonthly)}</div>
                    <div style={{ fontSize: 11, color: COLORS.textMuted, marginTop: 2 }}>보증금 {fmtWon(data.rentBase.medianDeposit)}</div>
                  </div>
                )}
              </>
            )}
          ...
          {/* 신뢰도 한 줄 */}
          {(data.rentBase.totalArticles > 0 || data.rentBase.dongCount > 0) && (
            <div style={{
              fontSize: 11, color: COLORS.textMuted, marginBottom: 10,
              padding: '6px 10px', background: 'rgba(255,255,255,0.03)', borderRadius: 8,
            }}>
              매물 {data.rentBase.totalArticles.toLocaleString()}건 · 주변 {data.rentBase.filteredDongCount || data.rentBase.dongCount}개 동 비교
              {data.rentBase.avgArea > 0 ? ` · 평균 ${(data.rentBase.avgArea / 3.3058).toFixed(1)}평 기준` : ''}
              {data.rentBase.perPyeong > 0 ? ` · 평당 월세 ${data.rentBase.perPyeong.toLocaleString()}만원` : ''}
            </div>
          )}

          {/* 주변 동 월세 비교 (낮은 순 TOP) */}
          {Array.isArray(data.rentBase.nearbyDongs) && data.rentBase.nearbyDongs.length > 0 && (
            <div style={{ background: 'rgba(255,255,255,0.04)', borderRadius: 10, padding: '12px 14px', border: '1px solid rgba(255,255,255,0.08)' }}>
              <div style={{ fontSize: 11, color: COLORS.textMuted, marginBottom: 8, fontWeight: 600 }}>주변 동 월세 (낮은 순)</div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                {data.rentBase.nearbyDongs.slice(0, 5).map((nd, i) => (
                  <div key={i} style={{
                    display: 'flex', justifyContent: 'space-between', alignItems: 'center',
                    fontSize: 12, color: COLORS.white, padding: '4px 0',
                    borderBottom: i < 4 ? '1px solid rgba(255,255,255,0.06)' : 'none',
                  }}>
                    <span style={{ color: COLORS.textSecondary }}>{nd.dong}</span>
                    <span>월 {fmtWon(nd.monthly)} · 보증금 {fmtWon(nd.deposit)}</span>
                  </div>
                ))}
              </div>
            </div>
...
const ChartWeatherImpact = ({ data }) => {
  // [2026-05-12] \uB370\uC774\uD130 \uC5C6\uC73C\uBA74 \uCC28\uD2B8 \uC790\uCCB4 null \uBC18\uD658 (0\uC77C \uB9C9\uB300 \uAC00\uC9DC \uD45C\uC2DC \uAE08\uC9C0)
  const hasData = data && Array.isArray(data.items) && data.items.length > 0
    && data.items.some(it => Number(it.value) > 0);
  if (!hasData) return null;
  const items = data.items;
...
const ChartInsightDashboard = ({ data }) => {
  ...
  // [2026-05-12] 빈 폴백 제거: 데이터 없으면 게이지/레이더/헤드라인 자체 숨김
  const headline = hasData && data.headline ? String(data.headline) : '';
  ...
  return (
    <div style={{
      ...
      {/* Radar Chart (radarData 비면 차트 자체 숨김) */}
      {radarData.length > 0 && (
...
const LABEL_MAP = {
  ...
  rentPerPyeong: '월 임대료', deposit: '보증금', supportPrograms: '지원 프로그램',
  ...
  // [2026-05-12] Card 10 영문 변수명 노출 방지
  positiveRatio: '긍정 비율', negativeRatio: '부정 비율', sentimentObj: '감성 분포',
...
const formatValue = (key, val) => {
  ...
  if (key === 'positiveRatio' || key === 'negativeRatio') return `${val}%`;
  if (key === 'weekday' || key === 'weekend') return `${val.toLocaleString()}명`;
  if (key === 'rentPerPyeong') {
    // [2026-05-12] 라벨 "월 임대료" -> 값에 "/평" 자동 추가 제거 (총 월세 표시)
    const v = Number(val);
    if (!v || v <= 0) return '-';
    if (v >= 10000) { const e = Math.floor(v / 10000); const m = Math.round(v % 10000); return m > 0 ? `${e}억 ${m.toLocaleString()}만원` : `${e}억원`; }
    return `${v.toLocaleString()}만원`;
...
export default function UnifiedLayout({
  ...
  // ── 한국부동산원/국세청/한국은행 신규 박스 데이터 (kosisExternal) ──
  // 우리 상권 매칭 → 시도 평균 폴백 → 전국 평균 폴백 순서로 값을 채운다.
  const kosisBoxData = useMemo(() => {
    if (!collectedData) return null;
    const apis = collectedData.apis || {};
    if (!apis.kosisExternal) return null;
    const addr = collectedData?.addressInfo?.address
      || collectedData?.address
      || searchAddress
      || '';
    // 시도 / 시군구 추출
    // [2026-05-12] addressInfo.sigungu 우선 사용 (이미 정확히 매핑됨). 자체 추출은 폴백.
    // 정규식 강화: "출구/입구/번/호" 같은 비행정 키워드 제외 + 시도 prefix 제외 후 시군구만
    const sidoMatch = addr.match(/^(서울|부산|대구|인천|광주|대전|울산|세종|경기|강원|충북|충남|전북|전남|경북|경남|제주)/);
    const sido = sidoMatch ? sidoMatch[1] : '';
    const _sigunguFromInfo = collectedData?.addressInfo?.sigungu || '';
    let sigungu = _sigunguFromInfo;
    if (!sigungu) {
      // 자체 추출: "출구|입구|번|호" 같은 부속어 매칭 방지
      // 시도 prefix 뒤의 첫 단어가 시/군/구로 끝나야 인정
      const _addrWithoutSido = sidoMatch ? addr.slice(sidoMatch[0].length).trim() : addr;
      const _sgMatch = _addrWithoutSido.match(/^([가-힣]+(?:시|군|구))/);
      sigungu = _sgMatch ? _sgMatch[1] : '';
    }

    // 우리 상권 코드 (없으면 시도 평균, 그것도 없으면 null)
    const sangkwon = mapToCommercialDistrict(addr);

    // 1차: 우리 상권 → 2차: null (extractor 내부에서 자동 폴백)
    const tryGet = (fn, key) => {
      const v1 = fn(apis, sangkwon);
      if (v1 && v1.value != null) return { ...v1, scope: '우리상권' };
      const v2 = fn(apis, null); // 전국/평균 폴백
      if (v2 && v2.value != null) return { ...v2, scope: '시도평균' };
      return null;
    };
    ...
    return {
      ...
      sigungu,
      address: addr,
      marketRent: tryGet(extractMarketRent),
      vacancy: tryGet(extractVacancy),
      priceChange: tryGet(extractPriceChange),
      conversionRate: tryGet(extractConversionRate),
      yieldRate: tryGet(extractYieldRate),
      netIncome: tryGet(extractNetIncome),
      cafeClosure: extractCafeClosure(apis, sido) || extractCafeClosure(apis, ''),
      cafeClosureNation: extractCafeClosure(apis, ''),
      regionClosure: extractRegionClosure(apis, sigungu) || extractRegionClosure(apis, ''),
      consumerSentiment: extractConsumerSentiment(apis, sido) || extractConsumerSentiment(apis, ''),
      // 자체 수집기(네이버부동산) + 한국부동산원 통합 임대료 (만원/평)
      integratedRent: buildIntegratedRent(apis, sangkwon) || buildIntegratedRent(apis, null),
      // ── 시계열 5종 (분기/월/연 추이) ──
      marketRentSeries: extractMarketRentSeries(apis, sangkwon, 8) || extractMarketRentSeries(apis, null, 8),
      vacancySeries: extractVacancySeries(apis, sangkwon, 8) || extractVacancySeries(apis, null, 8),
      priceIndexSeries: extractPriceIndexSeries(apis, sangkwon, 12) || extractPriceIndexSeries(apis, null, 12),
      cafeClosureSeries: extractCafeClosureSeries(apis, sido, 11) || extractCafeClosureSeries(apis, '', 11),
      consumerSentimentSeries: extractConsumerSentimentSeries(apis, sido, 12) || extractConsumerSentimentSeries(apis, '', 12),
    };
  ...
  const [subtitleSpeaker, setSubtitleSpeaker] = useState(null); // 'Director' | 'Client' | null
  const [subtitleText, setSubtitleText] = useState('');
  const [subtitleProgress, setSubtitleProgress] = useState(0); // 0~1
  const audioRef = useRef(null);

  // ── 팟캐스트 TTS 백그라운드 사전 생성: director 준비되면 자동으로 음성 미리 생성 ──
  useEffect(() => {
    try {
      const ic = (cards || []).find(c => c?.metaInfo === 'AI종합');
      const dr = ic?.chartData?.director;
      if (!dr || !dr.intro || !dr.market) return;
      // 검색마다 갱신을 위해 현재 director의 시그니처 비교
      const sig = (dr.intro || '') + '|' + (dr.market?.headline || '') + '|' + (dr.closing || '');
      if (window.__pregenTTSSig === sig && window.__pregenTTS) return;
      if (window.__pregenTTSPending === sig) return;
      window.__pregenTTSPending = sig;
      const _line = (sec) => {
        if (!sec) return '';
        if (typeof sec === 'string') return sec;
        const head = sec.headline || '';
        const obs = Array.isArray(sec.observations) ? sec.observations.join(' ') : '';
        return [head, obs].filter(Boolean).join('. ');
      };
      const _intro = (typeof dr.intro === 'string' && dr.intro.trim()) ? dr.intro.trim() : '안녕하세요. 이번 상권 분석 결과를 함께 살펴볼게요.';
      const _closing = (typeof dr.closing === 'string' && dr.closing.trim()) ? dr.closing.trim() : '오늘 살펴본 내용을 참고해서 결정해 보세요.';
      const _N = numbersToKorean;

      // [TTS 보강] 대시보드 풍부 정보를 디렉터 답변 끝에 한 줄씩 추가
      const _ttsCards = cards || [];
      const _findCard = (m) => _ttsCards.find(c => c?.metaInfo === m);
      const _c1 = _findCard('카페 현황');
      const _c2 = _findCard('고객');
      const _c3 = _findCard('프랜차이즈');
      const _c5 = _findCard('매출');
      const _c8 = _findCard('임대');
      const _c10 = _findCard('SNS');
      const _c13 = _findCard('상권변화');
      // 시장 보강: 카페 점포수 5년 변화
      const _marketExtra = (() => {
        const fiveY = _c13?.bodyData?.cafes5yAgo;
        const now = _c1?.bodyData?.cafes;
        if (fiveY && now) {
          const diff = now - fiveY;
          const sign = diff > 0 ? '+' : '';
          return `참고로 5년 전 ${fiveY}곳에서 지금 ${now}곳으로 ${sign}${diff}곳 변했습니다.`;
        }
        return '';
      })();
      // 고객 보강: 평균 소득 (customerYrEarn = { male, female, unit })
      const _customerExtra = (() => {
        const earn = _c2?.bodyData?.customerYrEarn;
        const m = earn?.male;
        const f = earn?.female;
        if (m && f) {
          const mRound = Math.round(m / 100) * 100;
          const fRound = Math.round(f / 100) * 100;
          return `방문 손님 연 평균 소득은 남성 약 ${mRound.toLocaleString()}만원, 여성 약 ${fRound.toLocaleString()}만원 수준입니다.`;
        }
        return '';
      })();
      // 경쟁 보강: 아메리카노 평균 (americanoAvg)
      const _compExtra = (() => {
        const am = _c1?.bodyData?.americanoAvg;
        if (am && am > 0) return `참고로 이 동네 아메리카노 평균 가격은 ${Number(am).toLocaleString()}원입니다.`;
        return '';
      })();
      // 수익 보강: 카페 평균 영업기간
      const _profitExtra = (() => {
        const yrs = _c13?.bodyData?.avgOperatingYears;
        if (yrs) return `이 동네 카페 평균 영업기간은 ${yrs}년입니다.`;
        return '';
      })();
      // 방향 보강: SNS 핫매장 1곳
      const _directionExtra = (() => {
        const top = _c10?.bodyData?.topShops;
        if (Array.isArray(top) && top.length > 0) {
          const first = top[0];
          const name = first?.name || first?.title || '';
          if (name) return `요즘 SNS에서 화제인 매장은 ${name} 같은 곳입니다.`;
        }
        return '';
      })();
      // 마무리 보강: 종합 등급
      const _closingExtra = (() => {
        const grade = (_findCard('AI종합'))?.chartData?.director?.heroMetric?.value;
        if (grade) return `종합 점수는 ${grade}입니다.`;
        return '';
      })();
      const _join = (a, b) => [a, b].filter(Boolean).join(' ');

      const script = `Director: ${_N(_intro)}\nClient: 아, 그렇군요. 좀 더 자세히 들려주시겠어요?\nDirector: ${_N(_join(_line(dr.market), _marketExtra)) || '카페 분포와 상권 구조를 살펴보고 있어요.'}\nClient: 그러면 그 안에서 손님들은 어떤 분들이 많아요?\nDirector: ${_N(_join(_line(dr.customer), _customerExtra)) || '연령과 라이프스타일을 확인하고 있어요.'}\nClient: 그런 손님들이라면 경쟁은 어떻게 봐야 할까요?\nDirector: ${_N(_join(_line(dr.competition), _compExtra)) || '경쟁 강도를 짚어보고 있어요.'}\nClient: 그래서 수익 측면에서는 어떤가요?\nDirector: ${_N(_join(_line(dr.profit), _profitExtra)) || '매출과 비용 구조를 살피고 있어요.'}\nClient: 그럼 어떤 방향으로 준비하는 게 맞을까요?\nDirector: ${_N(_join(_line(dr.direction), _directionExtra)) || '시장 흐름과 메뉴 트렌드를 정리하고 있어요.'}\nClient: 마지막으로 한마디로 정리해 주시겠어요?\nDirector: ${_N(_join(_closing, _closingExtra))}`;
      const apiKey = import.meta.env.VITE_GEMINI_API_KEY;
      if (!apiKey) { window.__pregenTTSPending = null; return; }
      console.log('[TTS] 팟캐스트 사전 생성 시작');
      fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-tts:generateContent?key=${apiKey}`, {
        ...
        if (audioData) {
          window.__pregenTTS = audioData;
          window.__pregenTTSScript = script;
          window.__pregenTTSSig = sig;
          console.log('[TTS] 팟캐스트 사전 생성 완료 (' + audioData.length + ' chars)');
        } else {
          console.log('[TTS] 사전 생성 응답 없음', result?.error?.message || '');
        }
      }).catch(e => {
        console.log('[TTS] 사전 생성 실패:', e.message);
      }).finally(() => {
        window.__pregenTTSPending = null;
      });
    } catch (e) {
      console.warn('[TTS] 사전 생성 useEffect 오류:', e.message);
    }
  }, [mappingResult]);
  ...
  return (
    ...
    >
      ...
      {showDirectorPopup && (() => {
        ...
        const dCard9 = cards?.find(c => c.metaInfo === '\uBC30\uB2EC\uAC1D\uB2E8\uAC00' || c.metaInfo === '\uBC30\uB2EC');
        ...
        // \uCD94\uAC00 \uCE74\uB4DC \uCC38\uC870 (\uAC1C\uC778\uCE74\uD398, \uCE74\uD398 \uAE30\uD68C)
        const dCard4 = cards?.find(c => c.metaInfo === '\uAC1C\uC778\uCE74\uD398');
        const dCard8 = cards?.find(c => c.metaInfo === '\uCE74\uD398 \uAE30\uD68C');
        ...
        const _rawBlockType = dCard1?.bodyData?.blockType || dCard1?.bodyData?.regionType || null;
        // 상권유형 자동 분류: 카페 수 + 유동인구 기반 (데이터 없을 때 빈값 방지)
        const blockType = _rawBlockType || (() => {
          const _t = dCard1?.bodyData?.cafes || dCard1?.chartData?.bigNumber || 0;
          const _p = dCard6?.bodyData?.totalPop || 0;
          if (_t >= 100 || _p >= 30000) return '핫플 상권';
          if (_t >= 30 || _p >= 5000) return '주거 상권';
          if (_t >= 10 || _p >= 1000) return '근린 상권';
          return '한적 상권';
        })();
        ...
        // 날씨 항목: chartData.items의 '맑음' 값 우선, 없으면 yearlyDistribution 계산
        const sunnyDays = (() => {
          const items = dCardWeather?.chartData?.items || [];
          const sunny = items.find(i => i.label === '맑음');
          if (sunny && sunny.value > 0) return sunny.value;
          const yd = dCardWeather?.bodyData?.yearlyDistribution;
          if (yd && yd.totalDays && yd.sunnyPct) return Math.round(yd.totalDays * yd.sunnyPct / 100);
          return null;
        })();
        const rainImpact = (() => {
          // 우선순위 1: 카드 11 rainyEffect (예: "-12%")
          const re = dCardWeather?.bodyData?.rainyEffect;
          if (re && typeof re === 'string') return re;
          // 우선순위 2: 카드 13 weatherSalesImpact 파싱
        ...
        // ── 추가 추출: 시장 탭용 ──
        // 1) 시간대 유동인구 미니 차트 (Card 6) - chartData 우선, 없으면 bodyData 폴백
        const hourlyChart = (() => {
          const _check = (labels, values) => {
            if (Array.isArray(labels) && Array.isArray(values) && labels.length > 0 && values.length === labels.length) {
              const max = Math.max(...values);
              if (max > 0) return { labels, values, max };
            }
            return null;
          };
          // 1순위: chartData (floatingTime API)
          const c1 = _check(dCard6?.chartData?.labels, dCard6?.chartData?.values);
          if (c1) return c1;
          // 2순위: bodyData.hourlyPctChart (소상공인 population 시간대 비중)
          const c2 = _check(dCard6?.bodyData?.hourlyPctChart?.labels, dCard6?.bodyData?.hourlyPctChart?.values);
          if (c2) return c2;
          // 3순위: bodyData.bizmapHourlyChart (비즈맵 카페 매출 집중 시간대)
          const c3 = _check(dCard6?.bodyData?.bizmapHourlyChart?.labels, dCard6?.bodyData?.bizmapHourlyChart?.values);
          if (c3) return c3;
          return null;
        })();
        // 2) 인기 메뉴 TOP 3 (Card 13 popularMenus 우선, 없으면 Card 3 popularMenuTop10)
        const popularMenusTop3 = (() => {
          const _pm = dCard13?.bodyData?.popularMenus;
          if (Array.isArray(_pm) && _pm.length > 0) {
            return _pm.slice(0, 3).map(m => ({ name: m.name, price: m.avgPrice || m.price || 0, salesRate: m.salesRate || 0 }));
          }
          const _pm2 = dCard3?.bodyData?.popularMenuTop10 || dCard3?.chartData?.popularMenuTop10;
          if (Array.isArray(_pm2) && _pm2.length > 0) {
            return _pm2.slice(0, 3).map(m => ({ name: m.name, price: m.price || 0, salesRate: 0 }));
          }
          return [];
        })();
        // 3) 점포수 5년 변화 (Card 13)
        const storesYearChange = (() => {
          const now = dCard13?.bodyData?.cafesNow;
          const past = dCard13?.bodyData?.cafes5yAgo;
          const rate = dCard13?.bodyData?.cafes5yChangeRate;
          if (now && past) return { now, past, rate: rate || 0 };
          return null;
        })();

        // ── 추가 추출: 경쟁 탭용 ──
        // 1) 프랜차이즈 TOP 5 (collectedData.nearbyFranchiseCounts)
        const franchiseTop5 = (() => {
          const fc = collectedData?.nearbyFranchiseCounts;
          if (fc && typeof fc === 'object') {
            const arr = Object.entries(fc)
              .filter(([, v]) => v > 0)
              .sort((a, b) => b[1] - a[1])
              .slice(0, 5)
          ...
          // 폴백: dCard3.chartData.items
          const items = dCard3?.chartData?.items;
          if (Array.isArray(items) && items.length > 0) {
            return items.slice(0, 5).map(it => ({ name: it.name, count: it.count || 0 }));
          }
          return [];
        })();
        // 2) 인기 급상승 메뉴 (Card 13 risingMenus)
        const risingMenusTop3 = (() => {
          const _rm = dCard13?.bodyData?.risingMenus;
          if (Array.isArray(_rm) && _rm.length > 0) {
            return _rm.slice(0, 3).map(m => ({ name: m.name, growth: m.growthRate || m.changeRate || 0 }));
          }
          return [];
        })();
        // 3) 5축 경쟁 점수 (Card 12 bodyData)
        // [2026-05-11] 비용 축이 0점일 수 있으므로 value>0 필터 제거 → 5축 모두 항상 노출
        const fiveAxisScores = (() => {
          const b = dCard12?.bodyData;
          if (!b) return null;
          // scoreBase 폴백: 외부지표 가산 전 원점수도 사용
          const _sb = b.scoreBase || {};
          const _pick = (a, b2) => (typeof a === 'number' ? a : (typeof b2 === 'number' ? b2 : null));
          const axes = [
            { name: '시장', value: _pick(b.scoreMarket, _sb.market) },
            { name: '경쟁', value: _pick(b.scoreCompete, _sb.compete) },
            { name: '변화', value: _pick(b.scoreChange, _sb.change) },
            { name: '생존', value: _pick(b.scoreSurvival, _sb.survival) },
            { name: '비용', value: _pick(b.scoreCost, _sb.cost) },
          ].filter(a => typeof a.value === 'number');
          if (axes.length === 0) return null;
          return { axes, total: b.score || 0 };
        })();

        // ── 추가 추출: 생존 탭용 ──
        // 1) 인테리어 비용 분포 (Card 8 또는 Card 7 chartData.kosisCafe)
        const interiorDist = (() => {
          const _src = dCard7?.chartData?.kosisCafe || cards?.find(c => c.chartType === 'kosisCafeFull')?.chartData?.kosisCafe;
          const dist = _src?.interiorDistribution?.distribution;
          if (Array.isArray(dist) && dist.length > 0) {
            return dist.slice(0, 5).map(d => ({ range: d.range, pct: d.normalizedPct || d.pct || 0 }));
          }
          return [];
        })();
        // 2) 평일/주말 매출 비중 (Card 6 weekday/weekend 또는 Card 9 weekdaySales)
        const weekdayWeekendSplit = (() => {
          const wd = dCard6?.bodyData?.weekday;
          const we = dCard6?.bodyData?.weekend;
          if (wd > 0 && we > 0) {
            const total = wd + we;
            return { weekdayPct: Math.round((wd / total) * 100), weekendPct: Math.round((we / total) * 100), source: '유동인구' };
          }
          // Card 6 weekdayPct/weekendPct
          const wp = dCard6?.bodyData?.weekdayPct;
          const ep = dCard6?.bodyData?.weekendPct;
          if (wp > 0 && ep > 0) return { weekdayPct: wp, weekendPct: ep, source: '유동인구' };
          return null;
        })();
        // 3) SNS 키워드 TOP 5 (Card 10 keywords)
        const dCard10 = cards?.find(c => c.metaInfo === 'SNS');
        const snsKeywordsTop5 = (() => {
          const kw = dCard10?.bodyData?.keywords;
          if (Array.isArray(kw) && kw.length > 0) return kw.slice(0, 5);
          const wc = dCard10?.chartData?.keywords;
          if (Array.isArray(wc) && wc.length > 0) {
            return wc.slice(0, 5).map(k => typeof k === 'string' ? k : (k.text || k.name || ''));
          }
          return [];
        })();

        // ── 추가 추출 v2 (2026-05-11): 13개 카드 데이터 누락 없이 모달에 배치 ──
        // [시장 탭] 방문 손님 평균소득 (Card 2 customerYrEarn)
        const customerYrEarn = (() => {
          const e = dCard2?.bodyData?.customerYrEarn;
          if (e && (e.male > 0 || e.female > 0)) {
            return { male: e.male || 0, female: e.female || 0 };
          }
          return null;
        })();

        // [경쟁 탭] 개인 카페 TOP 5 (Card 4 topNearbyIndie)
        const indieTop5 = (() => {
          const t = dCard4?.bodyData?.topNearbyIndie;
          if (Array.isArray(t) && t.length > 0) {
            return t.slice(0, 5).map(c => ({ name: c.name, dist: c.dist || 0 }));
          }
          return [];
        })();

        // [경쟁 탭] 동별 매출 TOP 5 (Card 5 topFiveDongsList)
        // dataMapper 출력 필드: { name, stores, amt, warning } - amt(만원 단위)가 정답
        const dongSalesTop5 = (() => {
          const t = dCard5?.bodyData?.topFiveDongsList;
          if (Array.isArray(t) && t.length > 0) {
            return t.slice(0, 5).map(d => ({
              name: d.name || d.admiNm || d.dongNm || '',
              amount: d.amt || d.amount || d.mmavgSlsAmt || d.value || 0,
            })).filter(x => x.name && x.amount > 0);
          }
          return [];
        })();

        // [경쟁 탭] 평균 영업기간 (Card 13 avgOperatingYears)
        const avgOperatingYears = dCard13?.bodyData?.avgOperatingYears || 0;

        // [경쟁 탭] 카페 종합 등급 (insightCard chartData.overall + recommendation)
        const overallGradeInfo = (() => {
          const score = insightCard?.chartData?.overall || insightCard?.bodyData?.overallScore || 0;
          const rec = insightCard?.bodyData?.recommendation || null;
          if (score > 0) return { score, label: rec || '' };
          return null;
        })();

        // [생존 탭] 카페 기회 신호 (Card 8 findings)
        // dataMapper findings 비어있을 때 aiSummary를 단일 항목으로 폴백
        const opportunityFindings = (() => {
          const f = dCard8?.bodyData?.findings;
          if (Array.isArray(f) && f.length > 0) {
            const out = f.slice(0, 4).map(x => ({ axis: x.axis || '', text: x.text || '' })).filter(x => x.text);
            if (out.length > 0) return out;
          }
          // 폴백 1: aiSummary 한 줄 (LOCALDATA 응답 없을 때 등장하는 요약 문구)
          const sum = dCard8?.aiSummary;
          if (sum && typeof sum === 'string' && sum.trim() && !sum.includes('수집 중')) {
            return [{ axis: '요약', text: sum.trim() }];
          }
          return [];
        })();

        // [생존 탭] 동네 배달 매출 (Card 9 - 검색 동 객단가/매출/주문)
        const localDelivery = (() => {
          const b = dCard9?.bodyData;
          if (!b) return null;
          if (!b.searchAvgPrice && !b.searchSales && !b.searchOrders) return null;
          return {
            dongName: b.searchDongName || '',
            avgPrice: b.searchAvgPrice || 0,
            sales: b.searchSales || 0,
            orders: b.searchOrders || 0,
            weekday: Array.isArray(b.weekdaySales) ? b.weekdaySales.slice(0, 7) : [],
          };
        })();

        // [생존 탭] SNS 인기 카페 (Card 10 topShops)
        const snsTopShops = (() => {
          const s = dCard10?.bodyData?.topShops;
        ...
        // [생존 탭] 날씨 매출 영향 (Card 11 sunnyEffect/rainyEffect/cloudyEffect/snowEffect)
        const weatherImpactBoxes = (() => {
          const b = dCardWeather?.bodyData;
          if (!b) return [];
          const arr = [];
          if (b.sunnyEffect) arr.push({ label: '맑음', value: b.sunnyEffect });
          if (b.cloudyEffect) arr.push({ label: '흐림', value: b.cloudyEffect });
          if (b.rainyEffect) arr.push({ label: '비', value: b.rainyEffect });
          if (b.snowEffect) arr.push({ label: '눈', value: b.snowEffect });
          return arr;
        })();
        ...
        const handleTTS = async () => {
          ...
          const d = director;
          // 새 director 구조: market/customer/...는 객체 ({headline, observations[], keyMetric, citation})
          // 화자 발화 텍스트로 변환: headline + observations(자연스럽게 이어 붙임)
          const _line = (sec) => {
            if (!sec) return '';
            if (typeof sec === 'string') return sec;
            const head = sec.headline || '';
          ...
          const _market = _line(d.market);
          const _customer = _line(d.customer);
          const _competition = _line(d.competition);
          const _profit = _line(d.profit);
          const _direction = _line(d.direction);
          // 자막용 발화 단위(turns): 화자 + 텍스트 + 섹션 매핑 (highlight 동기화용)
          const _intro = (typeof d.intro === 'string' && d.intro.trim()) ? d.intro.trim() : '';
          const _closing = (typeof d.closing === 'string' && d.closing.trim()) ? d.closing.trim() : '';
          const turns = [
            _intro && { speaker: 'Director', text: _intro, section: 'market' },
            { speaker: 'Client', text: '음... 이 동네는 어떤 상권이에요?', section: 'market' },
            _market && { speaker: 'Director', text: _market, section: 'market' },
            { speaker: 'Client', text: '그럼 여기 오는 손님들은 어떤 분들이에요?', section: 'customer' },
            _customer && { speaker: 'Director', text: _customer, section: 'customer' },
            { speaker: 'Client', text: '아, 그러면 경쟁은 심한 편인가요?', section: 'competition' },
            _competition && { speaker: 'Director', text: _competition, section: 'competition' },
            { speaker: 'Client', text: '솔직히 수익이 걱정인데, 괜찮을까요?', section: 'profit' },
            _profit && { speaker: 'Director', text: _profit, section: 'profit' },
            { speaker: 'Client', text: '그러면 어떤 방향으로 준비하면 좋을까요?', section: 'direction' },
            _direction && { speaker: 'Director', text: _direction, section: 'direction' },
            { speaker: 'Client', text: '정리하면 어떻게 봐야 할까요?', section: 'direction' },
            _closing && { speaker: 'Director', text: _closing, section: 'direction' },
          ].filter(Boolean);
          const turnsTotalChars = turns.reduce((s, t) => s + t.text.length + 10, 0);
          // 섹션별 대략적인 시간 분할 (기존 highlight 동기화용)
          const hlSegments = [
            { section: 'market', text: _market.substring(0, 80) },
            { section: 'customer', text: _customer.substring(0, 80) },
            { section: 'competition', text: _competition.substring(0, 80) },
            { section: 'profit', text: _profit.substring(0, 80) },
            { section: 'direction', text: _direction.substring(0, 80) },
          ];
          ...
          const playWithHighlight = (audioBytes) => {
            ...
            audioRef.current.play().then(() => {
              ...
              // 자막 turn별 timing
              let turnCum = 0;
              const turnTimings = turns.map(t => {
                const ratio = (t.text.length + 10) / turnsTotalChars;
                const start = turnCum * totalDuration;
                turnCum += ratio;
                return { ...t, start, end: turnCum * totalDuration };
              });
              const onTimeUpdate = () => {
                ...
                // 자막 갱신
                const t = turnTimings.find(tt => ct >= tt.start && ct < tt.end);
                if (t) {
                  setSubtitleSpeaker(t.speaker);
                  setSubtitleText(t.text);
                  const local = (ct - t.start) / Math.max(0.01, t.end - t.start);
                  setSubtitleProgress(Math.max(0, Math.min(1, local)));
                }
                // 전체 진행도 별도 표기 가능 (현재는 turn 안의 진행)
              };
          ...
          setIsAudioLoading(true);
          // \uC790\uC5F0\uC2A4\uB7EC\uC6B4 \uD31F\uCE90\uC2A4\uD2B8 \uB300\uD654 \uC2A4\uD06C\uB9BD\uD2B8 (Client \u2194 Director 8\uD134) + \uC22B\uC790 \uD55C\uAE00 \uBCC0\uD658
          // (_intro / _closing \uC740 \uC704\uC5D0\uC11C \uC774\uBBF8 \uC815\uC758\uB428)
          const _N = numbersToKorean;
          const script = `Director: ${_N(_intro)}\nClient: \uC74C... \uC774 \uB3D9\uB124\uB294 \uC5B4\uB5A4 \uC0C1\uAD8C\uC774\uC5D0\uC694?\nDirector: ${_N(_market) || '\uCE74\uD398 \uBD84\uD3EC\uC640 \uC0C1\uAD8C \uAD6C\uC870\uB97C \uC0B4\uD3B4\uBCF4\uACE0 \uC788\uC5B4\uC694.'}\nClient: \uADF8\uB7FC \uC5EC\uAE30 \uC624\uB294 \uC190\uB2D8\uB4E4\uC740 \uC5B4\uB5A4 \uBD84\uB4E4\uC774\uC5D0\uC694?\nDirector: ${_N(_customer) || '\uC5F0\uB839\uACFC \uB77C\uC774\uD504\uC2A4\uD0C0\uC77C\uC744 \uD655\uC778\uD558\uACE0 \uC788\uC5B4\uC694.'}\nClient: \uC544, \uADF8\uB7EC\uBA74 \uACBD\uC7C1\uC740 \uC2EC\uD55C \uD3B8\uC778\uAC00\uC694?\nDirector: ${_N(_competition) || '\uACBD\uC7C1 \uAC15\uB3C4\uB97C \uC9DA\uC5B4\uBCF4\uACE0 \uC788\uC5B4\uC694.'}\nClient: \uC194\uC9C1\uD788 \uC218\uC775\uC774 \uAC71\uC815\uC778\uB370, \uAD1C\uCC2E\uC744\uAE4C\uC694?\nDirector: ${_N(_profit) || '\uB9E4\uCD9C\uACFC \uBE44\uC6A9 \uAD6C\uC870\uB97C \uC0B4\uD53C\uACE0 \uC788\uC5B4\uC694.'}\nClient: \uADF8\uB7EC\uBA74 \uC5B4\uB5A4 \uBC29\uD5A5\uC73C\uB85C \uC900\uBE44\uD558\uBA74 \uC88B\uC744\uAE4C\uC694?\nDirector: ${_N(_direction) || '\uC2DC\uC7A5 \uD750\uB984\uACFC \uBA54\uB274 \uD2B8\uB80C\uB4DC\uB97C \uC815\uB9AC\uD558\uACE0 \uC788\uC5B4\uC694.'}\nClient: \uC815\uB9AC\uD558\uBA74 \uC5B4\uB5BB\uAC8C \uBD10\uC57C \uD560\uAE4C\uC694?\nDirector: ${_N(_closing)}`;
        ...
        return (
          ...
          >
            ...
            >
              ...
              {/* ── 디렉터 인사말 (intro) ── */}
              {director?.intro ? (
                <div style={{ padding: '12px 20px', background: '#0b0b0b', borderBottom: '1px solid #222', flexShrink: 0 }}>
                  <div style={{ fontSize: 14, color: '#E5E7EB', fontWeight: 500, lineHeight: 1.5, fontFamily: 'Pretendard, sans-serif' }}>
                    {director.intro}
                  </div>
                </div>
              ) : null}
              ...
              {isPlaying && subtitleText ? (
                <div style={{ padding: '16px 20px', background: '#0a0a0a', borderBottom: '1px solid #222', flexShrink: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
                    <span style={{
                      fontSize: 11, fontWeight: 700, letterSpacing: '0.06em',
                      padding: '3px 10px', borderRadius: 6,
                      background: subtitleSpeaker === 'Director' ? '#1E3A8A' : '#374151',
                      color: '#fff',
                    }}>
                      {subtitleSpeaker === 'Director' ? '디렉터' : '의뢰인'}
                    </span>
                  ...
                  <div style={{
                    fontSize: 15, color: '#E5E7EB', fontWeight: 500, lineHeight: 1.6,
                    fontFamily: 'Pretendard, sans-serif', minHeight: 48,
                  }}>
                    {subtitleText}
                  </div>
                  <div style={{ marginTop: 10, height: 3, background: '#222', borderRadius: 2, overflow: 'hidden' }}>
                    <div style={{
                      width: (subtitleProgress * 100) + '%', height: '100%',
                      background: subtitleSpeaker === 'Director' ? '#3182F6' : '#9CA3AF',
              ...
              <div style={{ flex: 1, overflow: 'auto', padding: '16px 20px', fontFamily: 'Pretendard, sans-serif', background: '#000000' }}>
                ...
                <div data-highlight="market" style={sectionStyle('market')}>
                  ...
                  {(() => {
                    ...
                    const _marketCount = _marketBoxes.filter(Boolean).length;
                    if (_marketCount === 0) return null;
                    return (
                      <div style={{ display: 'grid', gridTemplateColumns: `repeat(${_marketCount}, 1fr)`, gap: 10 }}>
                        {totalCafes > 0 && (
                          <div style={metricCard}>
                            <span style={metricLabel}>{'\uCD1D \uCE74\uD398'}</span>
                            <span style={metricValue}>
                              {(activeHighlight === 'market') ? <AnimatedNumber value={totalCafes} active={true} suffix={'\uAC1C'} /> : (totalCafes + '\uAC1C')}
                            </span>
                          </div>
                        )}
                        {blockType && (
                        ...
                        {monthlyPop > 0 && (
                          <div style={metricCard}>
                            ...
                            <span style={metricValue}>
                              {(activeHighlight === 'market') ? <AnimatedNumber value={monthlyPop} active={true} suffix={'\uBA85'} /> : (fmtPop(monthlyPop) + '\uBA85')}
                            </span>
                          </div>
                        )}
                        {closedCount > 0 && (
                          <div style={metricCard}>
                            ...
                            <span style={{ ...metricValue, color: '#EF4444' }}>
                              {closedCount + '\uACF3'}
                            </span>
                          </div>
                        )}
                  ...
                  {director?.market?.headline ? (
                    <div style={{
                      ...
                      <div style={{ fontSize: 14, color: '#fff', fontWeight: 700, marginBottom: 10, lineHeight: 1.4 }}>
                        {director.market.headline}
                      </div>
                      {Array.isArray(director.market.observations) && director.market.observations.length > 0 && (
                        <ul style={{ margin: 0, padding: 0, listStyle: 'none', display: 'flex', flexDirection: 'column', gap: 6 }}>
                          {director.market.observations.filter(Boolean).map((line, i) => (
                            <li key={i} style={{ fontSize: 13, color: '#D1D5DB', lineHeight: 1.5, paddingLeft: 12, position: 'relative' }}>
                      ...
                      {director.market.keyMetric?.value && (
                        <div style={{ marginTop: 10, paddingTop: 10, borderTop: '1px dashed rgba(255,255,255,0.1)', display: 'flex', alignItems: 'baseline', gap: 8 }}>
                          <span style={{ fontSize: 11, color: '#9CA3AF' }}>{director.market.keyMetric.label || '\uD575\uC2EC \uC9C0\uD45C'}</span>
                          <span style={{ fontSize: 16, color: '#FCD34D', fontWeight: 700 }}>{director.market.keyMetric.value}</span>
                        </div>
                      )}
                    </div>
                  ) : null}
                ...
                <div data-highlight="customer" style={sectionStyle('customer')}>
                  ...
                  {director?.customer?.headline ? (
                    <div style={{
                      ...
                      {Array.isArray(director.customer.observations) && director.customer.observations.length > 0 && (
                        <ul style={{ margin: 0, padding: 0, listStyle: 'none', display: 'flex', flexDirection: 'column', gap: 6 }}>
                          {director.customer.observations.filter(Boolean).map((line, i) => (
                            <li key={i} style={{ fontSize: 13, color: '#D1D5DB', lineHeight: 1.5, paddingLeft: 12, position: 'relative' }}>
                      ...
                      {director.customer.keyMetric?.value && (
                        <div style={{ marginTop: 10, paddingTop: 10, borderTop: '1px dashed rgba(255,255,255,0.1)', display: 'flex', alignItems: 'baseline', gap: 8 }}>
                          <span style={{ fontSize: 11, color: '#9CA3AF' }}>{director.customer.keyMetric.label || '\uD575\uC2EC \uC9C0\uD45C'}</span>
                          <span style={{ fontSize: 16, color: '#F472B6', fontWeight: 700 }}>{director.customer.keyMetric.value}</span>
                        </div>
                      )}
                    </div>
                  ) : null}
                ...
                {/* \uC2DC\uC7A5 \uD0ED \uCD94\uAC00 \uBC15\uC2A4: \uC2DC\uAC04\uB300\uBCC4 \uC720\uB3D9\uC778\uAD6C + \uC778\uAE30 \uBA54\uB274 + \uC810\uD3EC\uC218 \uCD94\uC774 */}
                {(hourlyChart || popularMenusTop3.length > 0 || storesYearChange) && (
                  <div data-highlight="market" style={sectionStyle('market')}>
                    <div style={{ fontSize: 13, color: '#3182F6', fontWeight: 600, marginBottom: 12, letterSpacing: '0.04em' }}>{'\uC2DC\uC7A5 \uC2EC\uCE35 \uC9C0\uD45C'}</div>
                    <div style={{ display: 'grid', gridTemplateColumns: hourlyChart ? '1.2fr 1fr' : '1fr', gap: 12 }}>
                      {hourlyChart && (
                        <div style={{ background: '#0d1014', border: '1px solid #1f2937', borderRadius: 10, padding: '12px 14px' }}>
                          <div style={{ fontSize: 11, color: '#9CA3AF', marginBottom: 10 }}>{'\uC2DC\uAC04\uB300\uBCC4 \uC720\uB3D9\uC778\uAD6C'}</div>
                          <div style={{ display: 'flex', alignItems: 'flex-end', gap: 6, height: 90 }}>
                            {hourlyChart.labels.map((lbl, i) => {
                              const v = hourlyChart.values[i] || 0;
                              const h = Math.max(4, Math.round((v / hourlyChart.max) * 80));
                              const isPeak = v === hourlyChart.max;
                              return (
                                <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
                                  <span style={{ fontSize: 9, color: isPeak ? '#FCD34D' : '#6B7280', fontWeight: isPeak ? 700 : 500 }}>{fmtPop(v)}</span>
                                  <div style={{ width: '100%', height: h, background: isPeak ? '#3182F6' : '#1f2937', borderRadius: 3, transition: 'height 0.8s ease-out' }} />
                      ...
                      {popularMenusTop3.length > 0 && (
                        <div style={{ background: '#0d1014', border: '1px solid #1f2937', borderRadius: 10, padding: '12px 14px' }}>
                          <div style={{ fontSize: 11, color: '#9CA3AF', marginBottom: 10 }}>{'\uC778\uAE30 \uBA54\uB274 TOP 3'}</div>
                          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                            {popularMenusTop3.map((m, i) => (
                              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                <span style={{ fontSize: 12, color: '#FCD34D', fontWeight: 700, width: 14 }}>{i + 1}</span>
                                <span style={{ fontSize: 12, color: '#E5E7EB', flex: 1, fontWeight: 500 }}>{m.name}</span>
                                {m.price > 0 && <span style={{ fontSize: 11, color: '#9CA3AF' }}>{Number(m.price).toLocaleString() + '\uC6D0'}</span>}
                                {m.salesRate > 0 && <span style={{ fontSize: 10, color: '#3182F6', fontWeight: 600 }}>{m.salesRate + '%'}</span>}
                              </div>
                            ))}
                          </div>
                        </div>
                    ...
                    {storesYearChange && (
                      <div style={{ marginTop: 12, padding: '12px 14px', background: '#0d1014', border: '1px solid #1f2937', borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                          <span style={{ fontSize: 11, color: '#9CA3AF' }}>{'\uCE74\uD398 \uC810\uD3EC\uC218 5\uB144 \uBCC0\uD654'}</span>
                          <span style={{ fontSize: 12, color: '#D1D5DB' }}>{'5\uB144 \uC804 ' + storesYearChange.past + '\uAC1C \u2192 \uD604\uC7AC ' + storesYearChange.now + '\uAC1C'}</span>
                        </div>
                        <span style={{ fontSize: 18, color: storesYearChange.rate >= 0 ? '#3B82F6' : '#EF4444', fontWeight: 700 }}>
                          {(storesYearChange.rate >= 0 ? '+' : '') + storesYearChange.rate + '%'}
                        </span>
                      </div>
                    )}
                  </div>
                ...
                {/* \uC2DC\uC7A5 \uD0ED - \uBC29\uBB38 \uC190\uB2D8 \uD3C9\uADE0\uC18C\uB4DD (Card 2) */}
                {customerYrEarn && (customerYrEarn.male > 0 || customerYrEarn.female > 0) && (
                  <div data-highlight="market" style={sectionStyle('market')}>
                    <div style={{ fontSize: 13, color: '#3182F6', fontWeight: 600, marginBottom: 10, letterSpacing: '0.04em' }}>{'\uBC29\uBB38 \uC190\uB2D8 \uD3C9\uADE0\uC18C\uB4DD'}</div>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                      {customerYrEarn.male > 0 && (
                        <div style={{ background: '#0d1014', border: '1px solid #1f2937', borderRadius: 10, padding: '12px 14px', display: 'flex', flexDirection: 'column', gap: 4 }}>
                          <span style={{ fontSize: 11, color: '#9CA3AF' }}>{'\uB0A8\uC131 \uC5F0 \uD3C9\uADE0'}</span>
                          <span style={{ fontSize: 18, color: '#3182F6', fontWeight: 700 }}>{Number(customerYrEarn.male).toLocaleString() + '\uB9CC\uC6D0'}</span>
                        </div>
                      )}
                      {customerYrEarn.female > 0 && (
                        <div style={{ background: '#0d1014', border: '1px solid #1f2937', borderRadius: 10, padding: '12px 14px', display: 'flex', flexDirection: 'column', gap: 4 }}>
                          <span style={{ fontSize: 11, color: '#9CA3AF' }}>{'\uC5EC\uC131 \uC5F0 \uD3C9\uADE0'}</span>
                          <span style={{ fontSize: 18, color: '#F472B6', fontWeight: 700 }}>{Number(customerYrEarn.female).toLocaleString() + '\uB9CC\uC6D0'}</span>
                        </div>
                      )}
                    </div>
                  </div>
                ...
                {/* \uC2DC\uC7A5 \uD0ED \uC2E0\uADDC \uBC15\uC2A4: \uD3C9\uB2F9 \uC784\uB300\uB8CC(\uD55C\uAD6D\uBD80\uB3D9\uC0B0\uC6D0) + \uACF5\uC2E4\uB960(\uD55C\uAD6D\uBD80\uB3D9\uC0B0\uC6D0) */}
                {(kosisBoxData?.marketRent || kosisBoxData?.vacancy) && (
                  <div data-highlight="market" style={sectionStyle('market')}>
                    <div style={{ fontSize: 13, color: '#3182F6', fontWeight: 600, marginBottom: 12, letterSpacing: '0.04em' }}>{'\uD55C\uAD6D\uBD80\uB3D9\uC0B0\uC6D0 \uC0C1\uAD8C \uC9C0\uD45C'}</div>
                    <div style={{ display: 'grid', gridTemplateColumns: kosisBoxData?.marketRent && kosisBoxData?.vacancy ? '1fr 1fr' : '1fr', gap: 10 }}>
                      {kosisBoxData?.marketRent && (() => {
                        const mr = kosisBoxData.marketRent;
                        const mrManwon = Math.round(mr.value / 10000);
                        return (
                          <div style={{ background: '#0d1014', border: '1px solid rgba(49,130,246,0.28)', borderRadius: 10, padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 6 }}>
                            <span style={{ fontSize: 11, color: '#9CA3AF' }}>{'\uD3C9\uB2F9 \uC6D4\uC138'}</span>
                            <div style={{ display: 'flex', alignItems: 'baseline', gap: 6 }}>
                              <span style={{ fontSize: 22, color: '#3182F6', fontWeight: 700 }}>{mrManwon.toLocaleString('ko-KR')}</span>
                              <span style={{ fontSize: 12, color: '#9CA3AF' }}>{'\uB9CC\uC6D0/\uD3C9'}</span>
                            </div>
                            <span style={{ fontSize: 10, color: '#6B7280' }}>{(mr.region || '\uC2DC\uB3C4') + (mr.scope === '\uC2DC\uB3C4\uD3C9\uADE0' ? ' (\uC2DC\uB3C4 \uD3C9\uADE0)' : '') + (mr.period ? ' \u00B7 ' + mr.period : '')}</span>
                          </div>
                        );
                      })()}
                      {kosisBoxData?.vacancy && (() => {
                        const vc = kosisBoxData.vacancy;
                        let _col = '#10B981';
                        let _txt = '\uD65C\uAE30\uCC2C \uC0C1\uAD8C';
                        if (vc.value >= 10) { _col = '#EF4444'; _txt = '\uBE48 \uC810\uD3EC \uB9CE\uC74C'; }
                        else if (vc.value >= 5) { _col = '#F59E0B'; _txt = '\uBCF4\uD1B5'; }
                        return (
                          <div style={{ background: '#0d1014', border: '1px solid rgba(49,130,246,0.28)', borderRadius: 10, padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 6 }}>
                            <span style={{ fontSize: 11, color: '#9CA3AF' }}>{'\uACF5\uC2E4\uB960'}</span>
                            <div style={{ display: 'flex', alignItems: 'baseline', gap: 6 }}>
                              <span style={{ fontSize: 22, color: _col, fontWeight: 700 }}>{Number(vc.value).toFixed(1)}</span>
                              <span style={{ fontSize: 12, color: '#9CA3AF' }}>{'%'}</span>
                              <span style={{ fontSize: 11, color: _col, fontWeight: 600, marginLeft: 4 }}>{_txt}</span>
                            </div>
                            <span style={{ fontSize: 10, color: '#6B7280' }}>{(vc.region || '\uC2DC\uB3C4') + (vc.scope === '\uC2DC\uB3C4\uD3C9\uADE0' ? ' (\uC2DC\uB3C4 \uD3C9\uADE0)' : '') + (vc.period ? ' \u00B7 ' + vc.period : '')}</span>
                          </div>
                        );
                      })()}
                    </div>
                ...
                <div data-highlight="competition" style={sectionStyle('competition')}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                      {[
                        indieRatio > 0 ? { label: '\uAC1C\uC778\uCE74\uD398 \uBE44\uC728', value: indieRatio + '%' } : null,
                        fcBrandCount > 0 ? { label: '\uD504\uB79C\uCC28\uC774\uC988 \uBE0C\uB79C\uB4DC', value: fcBrandCount + '\uAC1C' } : null,
                        americanoPrice ? { label: '\uC544\uBA54\uB9AC\uCE74\uB178 \uD3C9\uADE0', value: Number(americanoPrice).toLocaleString() + '\uC6D0' } : null,
                        dessertPrice ? { label: '\uB514\uC800\uD2B8 \uD3C9\uADE0', value: Number(dessertPrice).toLocaleString() + '\uC6D0' } : null,
                  ...
                  {director?.competition?.headline ? (
                    <div style={{
                      ...
                      {Array.isArray(director.competition.observations) && director.competition.observations.length > 0 && (
                        <ul style={{ margin: 0, padding: 0, listStyle: 'none', display: 'flex', flexDirection: 'column', gap: 6 }}>
                          {director.competition.observations.filter(Boolean).map((line, i) => (
                            <li key={i} style={{ fontSize: 13, color: '#D1D5DB', lineHeight: 1.5, paddingLeft: 12, position: 'relative' }}>
                      ...
                      {director.competition.keyMetric?.value && (
                        <div style={{ marginTop: 10, paddingTop: 10, borderTop: '1px dashed rgba(255,255,255,0.1)', display: 'flex', alignItems: 'baseline', gap: 8 }}>
                          <span style={{ fontSize: 11, color: '#9CA3AF' }}>{director.competition.keyMetric.label || '핵심 지표'}</span>
                          <span style={{ fontSize: 16, color: '#A855F7', fontWeight: 700 }}>{director.competition.keyMetric.value}</span>
                        </div>
                      )}
                    </div>
                  ) : null}
                ...
                <div data-highlight="profit" style={sectionStyle('profit')}>
                  ...
                  {(() => {
                    ...
                    const _profitCount = _profitBoxes.filter(Boolean).length;
                    if (_profitCount === 0) return null;
                    return (
                      <div style={{ display: 'grid', gridTemplateColumns: `repeat(${_profitCount}, 1fr)`, gap: 10, marginBottom: 12 }}>
                        {monthlySales > 0 && (
                          <div style={metricCard}>
                            <span style={metricLabel}>{'\uCE74\uD398 \uC6D4\uD3C9\uADE0 \uB9E4\uCD9C'}</span>
                            <span style={metricValue}>
                              {(activeHighlight === 'profit') ? <AnimatedNumber value={monthlySales} active={true} suffix={'\uB9CC\uC6D0'} /> : (fmtNum(monthlySales) + '\uB9CC\uC6D0')}
                            </span>
                          </div>
                        )}
                        {avgRent > 0 && (
                          <div style={metricCard}>
                            ...
                            <span style={metricValue}>{avgRent.toLocaleString() + '\uB9CC\uC6D0'}</span>
                          </div>
                        )}
                        {survivalRate1y && (
                          <div style={metricCard}>
                            <span style={metricLabel}>{'1\uB144 \uC0DD\uC874\uC728'}</span>
                            <span style={{ ...metricValue, color: survivalRate1y >= 50 ? '#3B82F6' : '#EF4444' }}>
                              {survivalRate1y + '%'}
                            </span>
                          </div>
                        )}
                  ...
                  {(() => {
                    ...
                    let _deliveryText = '';
                    if (_cafeCat) _deliveryText = `카페 배달 ${_cafeCat.rank}위 (월 ${_cafeCat.count?.toLocaleString()}건)`;
                    else if (_dd?.available) _deliveryText = '카페 배달: 5순위 밖 (매장/테이크아웃 중심)';
                    let _weatherText = '';
                    if (sunnyDays) _weatherText += `맑은 날 ${sunnyDays}일/년`;
                    if (rainImpact) _weatherText += (_weatherText ? ', ' : '') + `비 영향 ${rainImpact}`;
                    const _showDelivery = !!_deliveryText;
                    const _showWeather = !!_weatherText;
                    if (!_showDelivery && !_showWeather) return null;
                    return (
                      <div style={{ display: 'grid', gridTemplateColumns: (_showDelivery && _showWeather) ? '1fr 1fr' : '1fr', gap: 10 }}>
                        {_showDelivery && (
                          <div style={{ background: '#111', borderRadius: 8, padding: '10px 12px', border: '1px solid #222' }}>
                            ...
                            <span style={{ fontSize: 12, color: '#ccc' }}>{_deliveryText}</span>
                          </div>
                        )}
                        {_showWeather && (
                          <div style={{ background: '#111', borderRadius: 8, padding: '10px 12px', border: '1px solid #222' }}>
                            <span style={{ fontSize: 11, color: '#666', display: 'block', marginBottom: 4 }}>{'기상'}</span>
                            <span style={{ fontSize: 12, color: '#ccc' }}>{_weatherText}</span>
                          </div>
                        )}
                      </div>
                    );
                  ...
                  {director?.profit?.headline ? (
                    <div style={{
                      ...
                      {Array.isArray(director.profit.observations) && director.profit.observations.length > 0 && (
                        <ul style={{ margin: 0, padding: 0, listStyle: 'none', display: 'flex', flexDirection: 'column', gap: 6 }}>
                          {director.profit.observations.filter(Boolean).map((line, i) => (
                            <li key={i} style={{ fontSize: 13, color: '#D1D5DB', lineHeight: 1.5, paddingLeft: 12, position: 'relative' }}>
                      ...
                      {director.profit.keyMetric?.value && (
                        <div style={{ marginTop: 10, paddingTop: 10, borderTop: '1px dashed rgba(255,255,255,0.1)', display: 'flex', alignItems: 'baseline', gap: 8 }}>
                          <span style={{ fontSize: 11, color: '#9CA3AF' }}>{director.profit.keyMetric.label || '\uD575\uC2EC \uC9C0\uD45C'}</span>
                          <span style={{ fontSize: 16, color: '#FCD34D', fontWeight: 700 }}>{director.profit.keyMetric.value}</span>
                        </div>
                      )}
                    </div>
                  ) : null}
                  {/* KOSIS \uAD8C\uB9AC\uAE08 \uBC15\uC2A4 (\uC911\uC18C\uBCA4\uCC98\uAE30\uC5C5\uBD80 \uC0C1\uAC00\uAC74\uBB3C\uC784\uB300\uCC28\uC2E4\uD0DC\uC870\uC0AC) */}
                  {dCard7?.chartData?.premium ? (() => {
                    const _p = dCard7.chartData.premium;
                    const _avg = _p.sidoAvg || _p.nationalAvg || 0;
                    const _scope = _p.sidoKey || '\uC804\uAD6D';
                    const _receivePct = _p.sidoAvg ? _p.sidoReceivePct : _p.nationalReceivePct;
                    if (!_avg) return null;
                    return (
                      <div style={{
                        marginTop: 10, padding: '12px 14px',
                        background: '#0d1014', border: '1px solid #1f2937', borderRadius: 10,
                        display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12,
                      }}>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                          <span style={{ fontSize: 11, color: '#6B7280' }}>{'\uAD8C\uB9AC\uAE08 (\uC911\uAE30\uBD80 ' + (_p.year || '2023') + ')'}</span>
                          <span style={{ fontSize: 11, color: '#9CA3AF' }}>{_scope + ' \uD3C9\uADE0 \u00B7 \uBC1B\uC744 \uC608\uC815 ' + (_receivePct || 0) + '%'}</span>
                        </div>
                        <span style={{ fontSize: 18, color: '#FCD34D', fontWeight: 700 }}>{_avg.toLocaleString() + '\uB9CC\uC6D0'}</span>
                      </div>
                    );
                  })() : null}
                </div>

                                {/* 경쟁 탭 추가 박스: 프랜차이즈 TOP 5 + 급상승 메뉴 + 5축 점수 */}
                {(franchiseTop5.length > 0 || risingMenusTop3.length > 0 || fiveAxisScores) && (
                  <div data-highlight="competition" style={sectionStyle('competition')}>
                    <div style={{ fontSize: 13, color: '#A855F7', fontWeight: 600, marginBottom: 12, letterSpacing: '0.04em' }}>{'경쟁 심층 지표'}</div>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                      {franchiseTop5.length > 0 && (
                        <div style={{ background: '#0d1014', border: '1px solid #1f2937', borderRadius: 10, padding: '12px 14px' }}>
                          <div style={{ fontSize: 11, color: '#9CA3AF', marginBottom: 10 }}>{'프랜차이즈 TOP 5'}</div>
                          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                            {franchiseTop5.map((f, i) => {
                              const max = Math.max(...franchiseTop5.map(x => x.count));
                              const pct = max > 0 ? Math.round((f.count / max) * 100) : 0;
                              return (
                                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                  <span style={{ fontSize: 11, color: '#A855F7', fontWeight: 700, width: 14 }}>{i + 1}</span>
                                  <span style={{ fontSize: 12, color: '#E5E7EB', width: 70, fontWeight: 500, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{f.name}</span>
                                  <div style={{ flex: 1, height: 8, background: '#1f2937', borderRadius: 4, overflow: 'hidden' }}>
                                    <div style={{ width: pct + '%', height: '100%', background: '#A855F7', borderRadius: 4, transition: 'width 0.8s ease-out' }} />
                                  </div>
                                  <span style={{ fontSize: 11, color: '#D1D5DB', fontWeight: 600, width: 28, textAlign: 'right' }}>{f.count + '개'}</span>
                                </div>
                              );
                            })}
                          </div>
                      ...
                      {risingMenusTop3.length > 0 && (
                        <div style={{ background: '#0d1014', border: '1px solid #1f2937', borderRadius: 10, padding: '12px 14px' }}>
                          <div style={{ fontSize: 11, color: '#9CA3AF', marginBottom: 10 }}>{'인기 급상승 메뉴'}</div>
                          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                            {risingMenusTop3.map((m, i) => (
                              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 10px', background: 'rgba(168, 85, 247, 0.08)', borderRadius: 6 }}>
                                <span style={{ fontSize: 11, color: '#10B981', fontWeight: 700 }}>{'▲'}</span>
                                <span style={{ fontSize: 12, color: '#E5E7EB', flex: 1, fontWeight: 500 }}>{m.name}</span>
                                {m.growth > 0 && <span style={{ fontSize: 11, color: '#10B981', fontWeight: 700 }}>{'+' + m.growth + '%'}</span>}
                              </div>
                            ))}
                          </div>
                        </div>
                    ...
                    {fiveAxisScores && (
                      <div style={{ marginTop: 12, padding: '12px 14px', background: '#0d1014', border: '1px solid #1f2937', borderRadius: 10 }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 10 }}>
                          <span style={{ fontSize: 11, color: '#9CA3AF' }}>{'5축 경쟁 점수 (시장/경쟁/변화/생존/비용)'}</span>
                          {fiveAxisScores.total > 0 && <span style={{ fontSize: 14, color: '#A855F7', fontWeight: 700 }}>{'총점 ' + fiveAxisScores.total + '/100'}</span>}
                        </div>
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 8 }}>
                          {fiveAxisScores.axes.map((a, i) => {
                            const v = Math.min(100, Math.max(0, a.value));
                            return (
                              <div key={i} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
                                <div style={{ position: 'relative', width: '100%', height: 60, background: '#1f2937', borderRadius: 6, overflow: 'hidden' }}>
                                  <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: v + '%', background: 'linear-gradient(180deg, #A855F7, #6D28D9)', transition: 'height 0.8s ease-out' }} />
                                  <span style={{ position: 'absolute', top: 4, left: 0, right: 0, textAlign: 'center', fontSize: 10, color: '#fff', fontWeight: 700 }}>{v}</span>
                                </div>
                                <span style={{ fontSize: 10, color: '#9CA3AF' }}>{a.name}</span>
                              </div>
                            );
                          })}
                        </div>
                ...
                {/* 경쟁 탭 추가: 개인 카페 TOP 5 + 동별 매출 TOP 5 */}
                {(indieTop5.length > 0 || dongSalesTop5.length > 0) && (
                  <div data-highlight="competition" style={sectionStyle('competition')}>
                    <div style={{ fontSize: 13, color: '#A855F7', fontWeight: 600, marginBottom: 12, letterSpacing: '0.04em' }}>{'주변 매장 · 동별 매출'}</div>
                    <div style={{ display: 'grid', gridTemplateColumns: indieTop5.length > 0 && dongSalesTop5.length > 0 ? '1fr 1fr' : '1fr', gap: 12 }}>
                      {indieTop5.length > 0 && (
                        <div style={{ background: '#0d1014', border: '1px solid #1f2937', borderRadius: 10, padding: '12px 14px' }}>
                          <div style={{ fontSize: 11, color: '#9CA3AF', marginBottom: 10 }}>{'가까운 개인 카페 TOP 5'}</div>
                          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                            {indieTop5.map((c, i) => (
                              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                <span style={{ fontSize: 11, color: '#A855F7', fontWeight: 700, width: 14 }}>{i + 1}</span>
                                <span style={{ fontSize: 12, color: '#E5E7EB', flex: 1, fontWeight: 500, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{c.name}</span>
                                {c.dist > 0 && <span style={{ fontSize: 11, color: '#9CA3AF', fontWeight: 500 }}>{c.dist + 'm'}</span>}
                              </div>
                            ))}
                          </div>
                        </div>
                      )}
                      {dongSalesTop5.length > 0 && (
                        <div style={{ background: '#0d1014', border: '1px solid #1f2937', borderRadius: 10, padding: '12px 14px' }}>
                          <div style={{ fontSize: 11, color: '#9CA3AF', marginBottom: 10 }}>{'동별 카페 매출 TOP 5'}</div>
                          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                            {dongSalesTop5.map((d, i) => {
                              const max = Math.max(...dongSalesTop5.map(x => x.amount));
                              const w = max > 0 ? Math.round((d.amount / max) * 100) : 0;
                              return (
                                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                  <span style={{ fontSize: 11, color: '#A855F7', fontWeight: 700, width: 14 }}>{i + 1}</span>
                                  <span style={{ fontSize: 11, color: '#E5E7EB', width: 70, fontWeight: 500, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{d.name}</span>
                                  <div style={{ flex: 1, height: 8, background: '#1f2937', borderRadius: 4, overflow: 'hidden' }}>
                                    <div style={{ width: w + '%', height: '100%', background: '#A855F7', borderRadius: 4, transition: 'width 0.8s ease-out' }} />
                                  </div>
                                  <span style={{ fontSize: 10, color: '#D1D5DB', fontWeight: 600, textAlign: 'right' }}>{(d.amount >= 10000 ? (d.amount / 10000).toFixed(1) + '억' : d.amount.toLocaleString() + '만')}</span>
                                </div>
                              );
                            })}
                          </div>
                ...
                {/* 경쟁 탭 추가: 평균 영업기간 + 카페 종합 등급 */}
                {(avgOperatingYears > 0 || overallGradeInfo) && (
                  <div data-highlight="competition" style={sectionStyle('competition')}>
                    <div style={{ display: 'grid', gridTemplateColumns: avgOperatingYears > 0 && overallGradeInfo ? '1fr 1fr' : '1fr', gap: 12 }}>
                      {avgOperatingYears > 0 && (
                        <div style={{ background: '#0d1014', border: '1px solid #1f2937', borderRadius: 10, padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 6 }}>
                          <span style={{ fontSize: 11, color: '#9CA3AF' }}>{'평균 영업기간'}</span>
                          <span style={{ fontSize: 22, color: '#A855F7', fontWeight: 700 }}>{avgOperatingYears + '년'}</span>
                          <span style={{ fontSize: 10, color: '#6B7280' }}>{'동네 카페 평균 (인허가 기준)'}</span>
                        </div>
                      )}
                      {overallGradeInfo && (
                        <div style={{ background: '#0d1014', border: '1px solid #1f2937', borderRadius: 10, padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 6 }}>
                          <span style={{ fontSize: 11, color: '#9CA3AF' }}>{'카페 종합 등급'}</span>
                          <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
                            <span style={{ fontSize: 22, color: '#A855F7', fontWeight: 700 }}>{overallGradeInfo.score + '점'}</span>
                            {overallGradeInfo.label && <span style={{ fontSize: 13, color: '#FCD34D', fontWeight: 600 }}>{overallGradeInfo.label}</span>}
                          </div>
                          <span style={{ fontSize: 10, color: '#6B7280' }}>{'14개 지표 종합 (100점 만점)'}</span>
                        </div>
                      )}
                    </div>
                  </div>
                ...
                {/* 경쟁 탭 신규 박스: 국세청 카페 폐업 (이 시도) */}
                {(() => {
                  const cc = kosisBoxData?.cafeClosure;
                  // value=0 또는 null이면 박스 자체 숨김
                  if (!cc || cc.value == null || Number(cc.value) <= 0) return null;
                  const ccValue = Number(cc.value);
                  ...
                  let cmpColor = '#9CA3AF';
                  let cmpWeight = 500;
                  if (ccNation && ccNation.value && cc.scope !== ccNation.scope) {
                    const ratioNum = ccValue / Number(ccNation.value);
                    multiplier = Math.round(ratioNum * 10) / 10;
                    if (multiplier < 1.0) {
                      cmpLabel = '전국 평균보다 적음 (' + multiplier + '배)';
                      cmpColor = '#10B981';
                    } else if (multiplier < 1.5) {
                      cmpLabel = '전국 평균과 비슷 (' + multiplier + '배)';
                      cmpColor = '#9CA3AF';
                    } else {
                      cmpLabel = '전국 평균보다 많음 (' + multiplier + '배)';
                      cmpColor = '#EF4444';
                      cmpWeight = 700;
                    }
                  }
                  return (
                    <div data-highlight="competition" style={sectionStyle('competition')}>
                      <div style={{ fontSize: 13, color: '#A855F7', fontWeight: 600, marginBottom: 10, letterSpacing: '0.04em' }}>{'카페 폐업 신호 (국세청 ' + (periodLabel || '작년') + ')'}</div>
                      <div style={{ background: '#0d1014', border: '1px solid rgba(239,68,68,0.4)', borderRadius: 10, padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 8 }}>
                        <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
                          <span style={{ fontSize: 26, color: '#EF4444', fontWeight: 700 }}>{ccValue.toLocaleString('ko-KR')}</span>
                          <span style={{ fontSize: 13, color: '#D1D5DB' }}>{'곳 폐업 (' + region + ')'}</span>
                        </div>
                        {cmpLabel && (
                          <span style={{ fontSize: 12, color: cmpColor, fontWeight: cmpWeight }}>{cmpLabel}</span>
                        )}
                        <span style={{ fontSize: 10, color: '#6B7280' }}>{'국세청 100대 생활밀접업종 (커피음료점)'}</span>
                      </div>
                    </div>
                  );
                })()}
                ...
                <div data-highlight="direction" style={sectionStyle('direction')}>
                  ...
                  <div style={{ fontSize: 12, color: '#aaa', marginBottom: 14 }}>{'\uC774 \uC9C0\uC5ED \uC2DC\uC7A5\uC5D0\uC11C \uCC38\uACE0\uD560 \uD575\uC2EC \uC9C0\uD45C'}</div>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10, marginBottom: 16 }}>
                    {(() => {
                      // \uC9C0\uC5ED \uB370\uC774\uD130 \uAE30\uBC18 \uB3D9\uC801 \uCE74\uB4DC 3\uAC1C (\uD558\uB4DC\uCF54\uB529 X, \uBE48\uD06C\uB798\uD504\uD2B8 \uC5B8\uAE09 X)
                      const cards3 = [];
                      // \uCE74\uB4DC 1: \uBA54\uB274 \uAC00\uACA9\uB300 (Card 3 \uBA54\uB274 \uD3C9\uADE0\uAC00)
                      if (americanoPrice || dessertPrice) {
                        const parts = [];
                        if (americanoPrice) parts.push(`\uC544\uBA54\uB9AC\uCE74\uB178 ${Number(americanoPrice).toLocaleString()}\uC6D0`);
                        if (dessertPrice) parts.push(`\uB514\uC800\uD2B8 ${Number(dessertPrice).toLocaleString()}\uC6D0`);
                        cards3.push({ title: '\uBA54\uB274 \uAC00\uACA9\uB300', desc: parts.join(' \u00B7 ') });
                      }
                      // \uCE74\uB4DC 2: \uCE74\uD398\uB2F9 \uC7A0\uC7AC\uACE0\uAC1D (\uC720\uB3D9\uC778\uAD6C / \uCE74\uD398 \uC218)
                      if (totalCafes > 0 && dailyPop > 0) {
                        const perCafe = Math.round(dailyPop / totalCafes);
                        cards3.push({ title: '\uCE74\uD398\uB2F9 \uC7A0\uC7AC\uACE0\uAC1D', desc: `\uC77C\uD3C9\uADE0 ${perCafe.toLocaleString()}\uBA85 (\uC720\uB3D9\uC778\uAD6C \u00F7 \uCE74\uD398 \uC218)` });
                      }
                      // \uCE74\uB4DC 3: \uB9E4\uCD9C \uB4F1\uAE09 \uB610\uB294 1\uB144 \uC0DD\uC874\uC728
                      if (salesGrade) {
                        const _gradeMap = { S: '\uC77C\uD3C9\uADE0 50\uB9CC\uC6D0 \uC774\uC0C1', A: '\uC77C\uD3C9\uADE0 40~50\uB9CC\uC6D0', B: '\uC77C\uD3C9\uADE0 30~40\uB9CC\uC6D0', C: '\uC77C\uD3C9\uADE0 20~30\uB9CC\uC6D0', D: '\uC77C\uD3C9\uADE0 20\uB9CC\uC6D0 \uBBF8\uB9CC' };
                        cards3.push({ title: `\uB9E4\uCD9C \uB4F1\uAE09 ${salesGrade}`, desc: _gradeMap[salesGrade] || '\uB3D9\uB124 \uD3C9\uADE0 \uB9E4\uCD9C \uC218\uC900' });
                      } else if (survivalRate1y) {
                        cards3.push({ title: '1\uB144 \uC0DD\uC874\uC728', desc: `${survivalRate1y} (\uC774 \uB3D9\uB124 \uCE74\uD398 \uAE30\uC900)` });
                      }
                      // \uD3F4\uBC31: \uB370\uC774\uD130 \uBD80\uC871 \uC2DC \uC784\uB300\uB8CC \uBD80\uB2F4\uB960
                      if (cards3.length < 3 && avgRent && _monthlySalesNum) {
                        const burdenPct = Math.round((avgRent / _monthlySalesNum) * 100);
                        if (burdenPct > 0) cards3.push({ title: '\uC784\uB300 \uBD80\uB2F4\uB960', desc: `\uC6D4\uB9E4\uCD9C \uB300\uBE44 \uC57D ${burdenPct}% (\uD3C9\uB2F9 ${avgRent.toLocaleString()}\uB9CC\uC6D0)` });
                      }
                      return cards3.slice(0, 3).map((item, idx) => (
                        <div key={idx} style={{
                  ...
                  {director?.direction?.headline ? (
                    <div style={{
                      ...
                      {Array.isArray(director.direction.observations) && director.direction.observations.length > 0 && (
                        <ul style={{ margin: 0, padding: 0, listStyle: 'none', display: 'flex', flexDirection: 'column', gap: 6 }}>
                          {director.direction.observations.filter(Boolean).map((line, i) => (
                            <li key={i} style={{ fontSize: 13, color: '#D1D5DB', lineHeight: 1.5, paddingLeft: 12, position: 'relative' }}>
                      ...
                      {director.direction.keyMetric?.value && (
                        <div style={{ marginTop: 10, paddingTop: 10, borderTop: '1px dashed rgba(255,255,255,0.1)', display: 'flex', alignItems: 'baseline', gap: 8 }}>
                          <span style={{ fontSize: 11, color: '#9CA3AF' }}>{director.direction.keyMetric.label || '\uD575\uC2EC \uC9C0\uD45C'}</span>
                          <span style={{ fontSize: 16, color: '#10B981', fontWeight: 700 }}>{director.direction.keyMetric.value}</span>
                        </div>
                      )}
                    </div>
                  ) : null}

                  {/* KOSIS \uBC30\uB2EC \uBC15\uC2A4 (\uC804\uAD6D \uCE74\uD398 \uBC30\uB2EC \uC6B4\uC601\uB960) */}
                  {(() => {
                    const _kd = dCard9?.bodyData?.kosisDelivery || dCard9?.chartData?.kosisDelivery;
                    if (!_kd || (!_kd.app && !_kd.agency && !_kd.overallUsePct)) return null;
                    const _usePct = Math.round(_kd.overallUsePct || 0);
                    const _appCost = _kd.app?.monthlyManwon || 0;
                    const _agencyCost = _kd.agency?.monthlyManwon || 0;
                    const _both = _kd.bothMonthlyManwon || (_appCost + _agencyCost);
                    return (
                      <div style={{
                        marginTop: 10, padding: '12px 14px',
                        background: '#0d1014', border: '1px solid #1f2937', borderRadius: 10,
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 8 }}>
                          <span style={{ fontSize: 11, color: '#6B7280' }}>{'\uC804\uAD6D \uCE74\uD398 \uBC30\uB2EC (\uD1B5\uACC4\uCCAD ' + (_kd.year || '') + ')'}</span>
                          <span style={{ fontSize: 14, color: '#10B981', fontWeight: 700 }}>{'\uC6B4\uC601\uB960 ' + _usePct + '%'}</span>
                        </div>
                        <div style={{ fontSize: 12, color: '#D1D5DB', lineHeight: 1.5 }}>
                          {'\uBC30\uB2EC\uC571 \uC6D4 ' + (_appCost ? _appCost.toLocaleString() + '\uB9CC ' : '') + '+ \uBC30\uB2EC\uB300\uD589 \uC6D4 ' + (_agencyCost ? _agencyCost.toLocaleString() + '\uB9CC' : '') + (_both ? ' = \uD569 ' + _both.toLocaleString() + '\uB9CC \uBD80\uB2F4' : '')}
                        </div>
                      </div>
                    );
                  })()}
                  ...
                  {director?.closing ? (
                    <div style={{
                      ...
                      background: 'linear-gradient(135deg, #1E3A8A22, #3182F622)',
                      borderRadius: 10, border: '1px solid #1E3A8A44'
                    }}>
                      <div style={{ fontSize: 11, color: '#93C5FD', fontWeight: 600, marginBottom: 6, letterSpacing: '0.04em' }}>
                        {'\uB514\uB809\uD130 \uB9C8\uBB34\uB9AC'}
                      </div>
                      <div style={{ fontSize: 14, color: '#E5E7EB', fontWeight: 500, lineHeight: 1.5 }}>
                        {director.closing}
                      </div>
                    </div>
                  ) : null}
                  {/* 생존 탭 추가 박스: 인테리어 비용 분포 + 평일/주말 비중 + SNS 키워드 */}
                  {(interiorDist.length > 0 || weekdayWeekendSplit || snsKeywordsTop5.length > 0) && (
                    <div style={{ marginTop: 14, padding: 14, background: '#0d1014', border: '1px solid #1f2937', borderRadius: 10 }}>
                      <div style={{ fontSize: 13, color: '#10B981', fontWeight: 600, marginBottom: 12, letterSpacing: '0.04em' }}>{'생존 심층 지표'}</div>
                      {interiorDist.length > 0 && (
                        <div style={{ marginBottom: 12 }}>
                          <div style={{ fontSize: 11, color: '#9CA3AF', marginBottom: 8 }}>{'전국 카페 인테리어 비용 분포'}</div>
                          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                            {interiorDist.map((d, i) => {
                              const max = Math.max(...interiorDist.map(x => x.pct));
                              const w = max > 0 ? Math.round((d.pct / max) * 100) : 0;
                              const isMax = d.pct === max;
                              return (
                                <div key={i} style={{ display: 'grid', gridTemplateColumns: '90px 1fr 36px', gap: 8, alignItems: 'center' }}>
                                  <span style={{ fontSize: 11, color: isMax ? '#10B981' : '#9CA3AF', fontWeight: isMax ? 700 : 500 }}>{d.range}</span>
                                  <div style={{ height: 16, background: '#1f2937', borderRadius: 3, overflow: 'hidden' }}>
                                    <div style={{ width: w + '%', height: '100%', background: isMax ? '#10B981' : 'rgba(16,185,129,0.4)', borderRadius: 3, transition: 'width 0.8s ease-out' }} />
                                  </div>
                                  <span style={{ fontSize: 11, color: isMax ? '#10B981' : '#D1D5DB', fontWeight: 600, textAlign: 'right' }}>{d.pct + '%'}</span>
                                </div>
                              );
                            })}
                          </div>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: weekdayWeekendSplit && snsKeywordsTop5.length > 0 ? '1fr 1fr' : '1fr', gap: 12 }}>
                        {weekdayWeekendSplit && (
                          <div style={{ background: '#000', border: '1px solid #1f2937', borderRadius: 8, padding: '10px 12px' }}>
                            <div style={{ fontSize: 11, color: '#9CA3AF', marginBottom: 8 }}>{'평일/주말 비중 (' + weekdayWeekendSplit.source + ')'}</div>
                            <div style={{ display: 'flex', height: 22, borderRadius: 5, overflow: 'hidden' }}>
                              <div style={{ width: weekdayWeekendSplit.weekdayPct + '%', background: '#10B981', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, color: '#fff', fontWeight: 700, transition: 'width 0.8s ease-out' }}>
                                {weekdayWeekendSplit.weekdayPct >= 15 ? '평일 ' + weekdayWeekendSplit.weekdayPct + '%' : ''}
                              </div>
                              <div style={{ width: weekdayWeekendSplit.weekendPct + '%', background: '#3182F6', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, color: '#fff', fontWeight: 700, transition: 'width 0.8s ease-out' }}>
                                {weekdayWeekendSplit.weekendPct >= 15 ? '주말 ' + weekdayWeekendSplit.weekendPct + '%' : ''}
                              </div>
                            </div>
                          </div>
                        )}
                        {snsKeywordsTop5.length > 0 && (
                          <div style={{ background: '#000', border: '1px solid #1f2937', borderRadius: 8, padding: '10px 12px' }}>
                            <div style={{ fontSize: 11, color: '#9CA3AF', marginBottom: 8 }}>{'SNS 키워드 TOP 5'}</div>
                            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                              {snsKeywordsTop5.map((kw, i) => (
                                <span key={i} style={{ fontSize: 11, color: '#D1D5DB', padding: '4px 9px', background: 'rgba(16,185,129,0.12)', border: '1px solid rgba(16,185,129,0.3)', borderRadius: 12, fontWeight: 500 }}>{'#' + kw}</span>
                              ))}
                            </div>
                          </div>
                        )}
                  ...
                  {/* 생존 탭 추가: 카페 기회 신호 (Card 8 findings) */}
                  {opportunityFindings.length > 0 && (
                    <div style={{ marginTop: 14, padding: 14, background: '#0d1014', border: '1px solid #1f2937', borderRadius: 10 }}>
                      <div style={{ fontSize: 13, color: '#10B981', fontWeight: 600, marginBottom: 10, letterSpacing: '0.04em' }}>{'카페 기회 신호 ' + opportunityFindings.length + '건'}</div>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                        {opportunityFindings.map((f, i) => (
                          <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 10, padding: '8px 10px', background: 'rgba(16,185,129,0.08)', borderRadius: 6 }}>
                            {f.axis && <span style={{ fontSize: 10, color: '#10B981', fontWeight: 700, padding: '2px 6px', background: 'rgba(16,185,129,0.15)', borderRadius: 4, flexShrink: 0 }}>{f.axis}</span>}
                            <span style={{ fontSize: 12, color: '#D1D5DB', lineHeight: 1.5, flex: 1 }}>{f.text}</span>
                          </div>
                        ))}
                      </div>
                    </div>
                  ...
                  {/* 생존 탭 추가: 동네 배달 매출 (Card 9 검색 동) */}
                  {localDelivery && (
                    <div style={{ marginTop: 14, padding: 14, background: '#0d1014', border: '1px solid #1f2937', borderRadius: 10 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 10 }}>
                        <span style={{ fontSize: 13, color: '#10B981', fontWeight: 600, letterSpacing: '0.04em' }}>{'동네 배달 매출 ' + (localDelivery.dongName || '')}</span>
                      </div>
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8, marginBottom: localDelivery.weekday.length > 0 ? 10 : 0 }}>
                        {localDelivery.avgPrice > 0 && (
                          <div style={{ background: '#000', border: '1px solid #1f2937', borderRadius: 8, padding: '10px 12px', display: 'flex', flexDirection: 'column', gap: 3 }}>
                            <span style={{ fontSize: 10, color: '#9CA3AF' }}>{'배달 객단가'}</span>
                            <span style={{ fontSize: 14, color: '#10B981', fontWeight: 700 }}>{Number(localDelivery.avgPrice).toLocaleString() + '원'}</span>
                          </div>
                        )}
                        {localDelivery.sales > 0 && (
                          <div style={{ background: '#000', border: '1px solid #1f2937', borderRadius: 8, padding: '10px 12px', display: 'flex', flexDirection: 'column', gap: 3 }}>
                            <span style={{ fontSize: 10, color: '#9CA3AF' }}>{'월 배달 매출'}</span>
                            <span style={{ fontSize: 14, color: '#10B981', fontWeight: 700 }}>{(() => {
                              // 원 단위 → 만원으로 변환 후 표기 (1억=10000만)
                              const manwon = Math.round(localDelivery.sales / 10000);
                              if (manwon >= 10000) return (manwon / 10000).toFixed(1) + '억';
                              if (manwon > 0) return manwon.toLocaleString() + '만';
                              return localDelivery.sales.toLocaleString() + '원';
                            })()}</span>
                          </div>
                        )}
                        {localDelivery.orders > 0 && (
                          <div style={{ background: '#000', border: '1px solid #1f2937', borderRadius: 8, padding: '10px 12px', display: 'flex', flexDirection: 'column', gap: 3 }}>
                            <span style={{ fontSize: 10, color: '#9CA3AF' }}>{'월 주문 수'}</span>
                            <span style={{ fontSize: 14, color: '#10B981', fontWeight: 700 }}>{Number(localDelivery.orders).toLocaleString() + '건'}</span>
                          </div>
                        )}
                      </div>
                      {localDelivery.weekday.length > 0 && (() => {
                        const max = Math.max(...localDelivery.weekday.map(d => d.amount || 0));
                        if (max <= 0) return null;
                        return (
                          <div>
                            <div style={{ fontSize: 10, color: '#9CA3AF', marginBottom: 6 }}>{'요일별 배달 매출'}</div>
                            <div style={{ display: 'flex', alignItems: 'flex-end', gap: 6, height: 50 }}>
                              {localDelivery.weekday.map((d, i) => {
                                const h = Math.max(4, Math.round(((d.amount || 0) / max) * 44));
                                return (
                                  <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3 }}>
                                    <div style={{ width: '100%', height: h, background: d.isTop ? '#10B981' : (d.isLow ? '#374151' : '#1f2937'), borderRadius: 3, transition: 'height 0.8s ease-out' }} />
                                    <span style={{ fontSize: 9, color: d.isTop ? '#10B981' : '#6B7280', fontWeight: d.isTop ? 700 : 500 }}>{d.day}</span>
                                  </div>
                                );
                              })}
                            </div>
                  ...
                  {/* 생존 탭 추가: SNS 인기 카페 (Card 10 topShops) */}
                  {snsTopShops.length > 0 && (
                    <div style={{ marginTop: 14, padding: 14, background: '#0d1014', border: '1px solid #1f2937', borderRadius: 10 }}>
                      <div style={{ fontSize: 13, color: '#10B981', fontWeight: 600, marginBottom: 10, letterSpacing: '0.04em' }}>{'SNS 인기 카페 ' + snsTopShops.length + '곳'}</div>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                        {snsTopShops.map((s, i) => (
                          <div key={i} style={{ padding: '8px 12px', background: 'rgba(16,185,129,0.06)', borderRadius: 6, borderLeft: '2px solid #10B981' }}>
                            <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, marginBottom: s.reason ? 4 : 0 }}>
                              <span style={{ fontSize: 12, color: '#E5E7EB', fontWeight: 700 }}>{s.name}</span>
                              {s.menu && <span style={{ fontSize: 11, color: '#9CA3AF' }}>{s.menu}</span>}
                            </div>
                            {s.reason && <span style={{ fontSize: 11, color: '#9CA3AF', lineHeight: 1.4 }}>{s.reason}</span>}
                          </div>
                        ))}
                      </div>
                    </div>
                  ...
                  {/* 생존 탭 추가: 날씨 매출 영향 (Card 11) */}
                  {weatherImpactBoxes.length > 0 && (
                    <div style={{ marginTop: 14, padding: 14, background: '#0d1014', border: '1px solid #1f2937', borderRadius: 10 }}>
                      <div style={{ fontSize: 13, color: '#10B981', fontWeight: 600, marginBottom: 10, letterSpacing: '0.04em' }}>{'날씨별 매출 영향'}</div>
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(' + weatherImpactBoxes.length + ', 1fr)', gap: 8 }}>
                        {weatherImpactBoxes.map((w, i) => {
                          const isPos = w.value && w.value.startsWith('+');
                          const isNeg = w.value && w.value.startsWith('-');
                          const color = isPos ? '#10B981' : isNeg ? '#EF4444' : '#9CA3AF';
                          return (
                            <div key={i} style={{ background: '#000', border: '1px solid #1f2937', borderRadius: 8, padding: '10px 8px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
                              <span style={{ fontSize: 10, color: '#9CA3AF' }}>{w.label}</span>
                              <span style={{ fontSize: 16, color: color, fontWeight: 700 }}>{w.value}</span>
                            </div>
                          );
                        })}
                      </div>
                  ...
                  {/* 생존 탭 신규 박스: 임대가격지수 변동 + 권역 소비심리 (한국부동산원/한국은행) */}
                  {(kosisBoxData?.priceChange || kosisBoxData?.consumerSentiment) && (
                    <div style={{ marginTop: 14, padding: 14, background: '#0d1014', border: '1px solid #1f2937', borderRadius: 10 }}>
                      <div style={{ fontSize: 13, color: '#10B981', fontWeight: 600, marginBottom: 10, letterSpacing: '0.04em' }}>{'시장 흐름 지표'}</div>
                      <div style={{ display: 'grid', gridTemplateColumns: kosisBoxData?.priceChange && kosisBoxData?.consumerSentiment ? '1fr 1fr' : '1fr', gap: 10 }}>
                        {kosisBoxData?.priceChange && (() => {
                          const pc = kosisBoxData.priceChange;
                          const v = pc.value;
                          let _col = '#9CA3AF';
                          let _txt = '변동 없음';
                          if (v != null) {
                            if (v > 0) { _col = '#EF4444'; _txt = '오름세'; }
                            else if (v < 0) { _col = '#10B981'; _txt = '내림세'; }
                          }
                          return (
                            <div style={{ background: '#000', border: '1px solid rgba(16,185,129,0.3)', borderRadius: 8, padding: '12px 14px', display: 'flex', flexDirection: 'column', gap: 5 }}>
                              <span style={{ fontSize: 11, color: '#9CA3AF' }}>{'임대료 1년 변동률'}</span>
                              <div style={{ display: 'flex', alignItems: 'baseline', gap: 6 }}>
                                <span style={{ fontSize: 20, color: _col, fontWeight: 700 }}>{v != null ? (v > 0 ? '+' : '') + Number(v).toFixed(1) + '%' : ''}</span>
                                <span style={{ fontSize: 11, color: _col, fontWeight: 600 }}>{_txt}</span>
                              </div>
                              <span style={{ fontSize: 10, color: '#6B7280' }}>{(pc.region || '시도') + (pc.scope === '시도평균' ? ' (시도 평균)' : '') + (pc.period ? ' · ' + pc.period : '')}</span>
                            </div>
                          );
                        })()}
                        {kosisBoxData?.consumerSentiment && (() => {
                          const cs = kosisBoxData.consumerSentiment;
                          let _col = '#F59E0B';
                          let _txt = '보통';
                          if (cs.value >= 100) { _col = '#10B981'; _txt = '낙관적'; }
                          else if (cs.value < 90) { _col = '#EF4444'; _txt = '비관적'; }
                          return (
                            <div style={{ background: '#000', border: '1px solid rgba(16,185,129,0.3)', borderRadius: 8, padding: '12px 14px', display: 'flex', flexDirection: 'column', gap: 5 }}>
                              <span style={{ fontSize: 11, color: '#9CA3AF' }}>{'권역 소비심리지수'}</span>
                              <div style={{ display: 'flex', alignItems: 'baseline', gap: 6 }}>
                                <span style={{ fontSize: 20, color: _col, fontWeight: 700 }}>{cs.value}</span>
                                <span style={{ fontSize: 11, color: '#9CA3AF' }}>{'점'}</span>
                                <span style={{ fontSize: 11, color: _col, fontWeight: 600, marginLeft: 4 }}>{_txt}</span>
                              </div>
                              <span style={{ fontSize: 10, color: '#6B7280' }}>{(cs.region || '전국') + (cs.scope === '전국평균' ? ' (전국 평균)' : '') + (cs.period ? ' · ' + cs.period : '') + ' · 한국은행'}</span>
                            </div>
                          );
                        })()}
                      </div>
                    ...
                    {cards.map((card, i) => {
                      ...
                      >
                        ...
                        {i === 0 ? (
                          ...
                          <Card5SalesAnalysis card={card} cardNumber={(i + 1) < 10 ? `0${i + 1}` : `${i + 1}`} kosisBoxData={kosisBoxData} />
                        ) : i === 6 ? (
                        ...
                        <CardTemplate
                          ...
                          bodyContent={
                            card.metaInfo === 'AI종합' ? (
                              ...
                              <div>
                                {/* 통합 임대료 박스 (자체 수집기 + 한국부동산원 가중평균) */}
                                {kosisBoxData?.integratedRent && (() => {
                                  const ir = kosisBoxData.integratedRent;
                                  const subTxt = ir.integrated
                                    ? `${ir.sources.join(' + ')} 가중평균`
                                    : `출처: ${ir.sources[0]}`;
                                  return (
                                    <div style={{
                                      marginBottom: 16,
                                      background: 'linear-gradient(135deg, rgba(49,130,246,0.14), rgba(30,58,138,0.10))',
                                      border: '1px solid rgba(49,130,246,0.28)',
                                      borderRadius: 14,
                                      padding: '16px 16px 14px',
                                    }}>
                                      <div style={{ fontSize: 12, fontWeight: 600, color: '#9CC1FF', marginBottom: 8, letterSpacing: '0.02em' }}>
                                        통합 임대 시세 {ir.integrated ? '(가중평균)' : ''}
                                      </div>
                                      <div style={{ display: 'flex', alignItems: 'baseline', gap: 6, marginBottom: 6 }}>
                                        <div style={{ fontSize: 28, fontWeight: 800, color: '#FFFFFF', lineHeight: 1 }}>
                                          {ir.value.toLocaleString('ko-KR')}
                                        </div>
                                        <div style={{ fontSize: 13, color: COLORS.textMuted, fontWeight: 500 }}>만원/평</div>
                                      </div>
                                ...
                                {/* 한국부동산원 핵심 4박스 (평당 월세 / 전환율 / 수익률 / 순영업소득) */}
                                {kosisBoxData && (kosisBoxData.marketRent || kosisBoxData.conversionRate || kosisBoxData.yieldRate || kosisBoxData.netIncome) && (() => {
                                  const mr = kosisBoxData.marketRent;
                                  const cr = kosisBoxData.conversionRate;
                                  const yr = kosisBoxData.yieldRate;
                                  const ni = kosisBoxData.netIncome;
                                  const mrManwon = mr ? Math.round(mr.value / 10000) : null; // 원/평 → 만원/평
                                  const niManwon = ni ? Math.round(ni.value / 10000) : null;
                                  // 보증금 1억 → 월세 환산: 1억 × 전환율(연%) / 12 / 100
                                  const depositToMonthly = cr ? Math.round((100000000 * (cr.value / 100)) / 12 / 10000) : null;
                                  const periodTxt = mr?.period || cr?.period || yr?.period || ni?.period || '';
                                  const regionTxt = mr?.region || cr?.region || yr?.region || ni?.region || '시도 평균';
                                  const scopeTxt = (mr?.scope === '시도평균' || cr?.scope === '시도평균') ? ' (시도 평균)' : '';
                                  const cellStyle = {
                                    background: 'rgba(255,255,255,0.04)',
                                    border: '1px solid rgba(255,255,255,0.08)',
                                    borderRadius: 12,
                                  ...
                                  const labelStyle = { fontSize: 11, color: COLORS.textMuted, fontWeight: 500, letterSpacing: '0.02em' };
                                  const valStyle = { fontSize: 20, fontWeight: 700, color: '#FFFFFF', lineHeight: 1.2 };
                                  const subStyle = { fontSize: 10, color: COLORS.textMuted, lineHeight: 1.4 };
                                  return (
                                    <div style={{ marginBottom: 16 }}>
                                      ...
                                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 10 }}>
                                        {mrManwon != null && (
                                          <div style={cellStyle}>
                                            <div style={labelStyle}>평당 월세</div>
                                            <div style={valStyle}>{mrManwon.toLocaleString('ko-KR')}<span style={{ fontSize: 12, color: COLORS.textMuted, marginLeft: 2 }}>만원/평</span></div>
                                            <div style={subStyle}>중대형 상가 기준</div>
                                          </div>
                                        )}
                                        {cr && (
                                          <div style={cellStyle}>
                                            <div style={labelStyle}>전환율 (연)</div>
                                            <div style={valStyle}>{Number(cr.value).toFixed(1)}<span style={{ fontSize: 12, color: COLORS.textMuted, marginLeft: 2 }}>%</span></div>
                                            <div style={subStyle}>보증금 1억 → 월세 약 {depositToMonthly != null ? `${depositToMonthly}만원` : '계산 불가'}</div>
                                          </div>
                                        )}
                                        {yr && (
                                          <div style={cellStyle}>
                                            <div style={labelStyle}>수익률 (종합)</div>
                                            <div style={valStyle}>{Number(yr.annual ?? yr.value).toFixed(2)}<span style={{ fontSize: 12, color: COLORS.textMuted, marginLeft: 2 }}>%</span></div>
                                            <div style={subStyle}>연 환산 (분기 {yr.quarterly != null ? Number(yr.quarterly).toFixed(2) : Number(yr.value).toFixed(2)}% × 4)</div>
                                          </div>
                                        )}
                                        {niManwon != null && (
                                          <div style={cellStyle}>
                                            <div style={labelStyle}>순영업소득</div>
                                            <div style={valStyle}>{niManwon.toLocaleString('ko-KR')}<span style={{ fontSize: 12, color: COLORS.textMuted, marginLeft: 2 }}>만원/평/년</span></div>
                                            <div style={subStyle}>임대료 - 운영비 차감</div>
                                          </div>
                                        )}
                                      </div>
                                      <div style={{ fontSize: 10, color: COLORS.textMuted, marginTop: 8, textAlign: 'right' }}>
                                        {regionTxt}{scopeTxt} · 한국부동산원 {periodTxt}
                                      </div>
                                    </div>
                                  );
                                })()}
                                <DataTable data={card.bodyData} chartType={card.chartType} />
                                <KosisCafeFullBox data={card.chartData} />
                              </div>
                            ) : (
                              <DataTable data={card.bodyData} chartType={card.chartType} />
                            )
