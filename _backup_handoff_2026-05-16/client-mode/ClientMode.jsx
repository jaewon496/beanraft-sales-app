...
export default function ClientMode({
  ...
  // ═══ [수집능력 고정 - 수정 금지] options 전달 경로 ═══
  // onSearchRegion(address, options) - options.poiCoords가 searchSalesModeRegion으로 전달됨
  const handleSearch = useCallback((address, radius, options) => {
    ...
    if (onSearchRegion) {
      onSearchRegion(address, options || undefined);
    } else {
      startLoading(address);
    }
  }, [startLoading, onSearchRegion]);

  // Sync real analysis progress to loading progress - 단조 증가 보장 (역행 방지)
  useEffect(() => {
    if (onSearchRegion && phase === PHASE.LOADING) {
      ...
      // 단조 증가: 현재 progress보다 작으면 무시 (95% → 85% 역행 방지)
      if (effectiveProgress >= progressRef.current) {
