...
const Card02DetailPopup = ({
  ...
  // SECTION 2/3/6/7 제거 (다른 카드와 중복) — 성별×연령 / 가구 / 주거 환경만 표시
  const sections = [
  ...
  const hasAny = (() => {
    ...
    const d5 = toNum(popData?.aptLiveRatio) > 0
      || (Array.isArray(popData?.hjdNames) && popData.hjdNames.length > 0)
      || !!popData?.ageGender
      || toNum(bodyData?.openubAptRatio) > 0;
    return d1 || d4 || d5;
  })();
