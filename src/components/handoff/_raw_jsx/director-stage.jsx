/* director-stage.jsx — 3탭 대시보드 본문 (Dr* 프리미티브 사용)
   ───────────────────────────────────────────────────────────── */
...
function MarketStage() {
  const { DrKpiTile, DrLineChart, DrBarChart, DrDonut, DrPulse } = window;
  ...
  return (
    <div className="dr-stage">
      ...
      <div className="dr-grid-4" style={{marginBottom: 14}}>
        <DrKpiTile id="m.cafeCount" label="반경 500m 카페" value={126} suffix="개" tone="blue" hero pulse src="소상공인진흥공단 2025.Q1"/>
        <DrKpiTile id="m.salesAvg"  label="월평균 매출"     value={9121} suffix="만원" tone="mint" hero pulse src="국세청 카드매출 2024" delta={16.3} deltaPositive/>
        <DrKpiTile id="m.foot"      label="일평균 유동인구"  value={578505} tone="lilac" hero pulse src="KT 빅데이터 2025.04"/>
        <DrKpiTile id="m.rent"      label="평당 월세"        value={42} suffix="만원" tone="cream" hero pulse src="한국부동산원 2025.Q1" delta={10.1} deltaPositive={false}/>
      </div>

      {/* mid row: sales line + hour bar */}
      <div className="dr-grid-2-3" style={{marginBottom: 14}}>
        <div className="dr-panel">
          ...
          <DrLineChart id="m.salesLine" data={salesData} color="#71DD8C" w={520} h={150} fill/>
          <div className="dr-line__foot">2024.06 → 2025.05 · 국세청 KOSIS</div>
        </div>
        <div className="dr-panel">
          <div className="dr-panel__title">시간대별 통행량 <span>피크: 18시</span></div>
          <DrBarChart id="m.hourBar" data={hourData} labels={hourLabels} color="#7DBBFF" h={150} highlightIdx={18}/>
          <div className="dr-line__foot">KT 빅데이터 2025.04</div>
        </div>
      </div>

      {/* bottom row: rent quarterly + cafe donut */}
      <div className="dr-grid-2-3">
        <div className="dr-panel">
          <div className="dr-panel__title">
            분기별 평당 월세 <span>최근 8분기</span>
            <DrPulse id="m.rentDelta" color="#EF4444">
              <span className="dr-chip bad">+10.1% / 1y</span>
            </DrPulse>
          </div>
          <DrLineChart id="m.rentLine" data={rentData} color="#EF4444" w={520} h={130} fill/>
        ...
        <div className="dr-panel">
          ...
          <DrDonut
            ...
            size={150}
            data={[
              { label: "개인 카페", value: 79, color: "#7DBBFF" },
              { label: "프랜차이즈", value: 47, color: "#A259FF" },
            ]}
...
/* ============================================================
   ...
   ============================================================ */
function CompStage() {
  const { DrKpiTile, DrBarChart, DrDonut, DrRadar, DrLiveCountUp } = window;
  ...
  return (
    <div className="dr-stage">
      <div className="dr-grid-2-3" style={{marginBottom: 14}}>
        ...
        <div className="dr-panel">
          ...
          <DrDonut
            ...
            size={140}
            data={[
              { label: "개인",        value: 79, color: "#7DBBFF" },
              { label: "프랜차이즈",   value: 47, color: "#A259FF" },
            ]}
      ...
      <div className="dr-grid-4" style={{marginBottom: 14}}>
        ...
        <DrKpiTile id="c.openClose" label="1년 신규 / 폐업" value={5} suffix=" / 8개" tone="cream" src="국세청 KOSIS" pulse/>
        ...
        <div className="dr-oc-bar">
          {openClose.map((d, i) => (
            <div key={i} className="dr-oc-bar__col">
              <DrBarChart id="c.openClose" data={[d.o, d.c]} h={70} color="#71DD8C"/>
  ...
  return (
    <div className="dr-stage">
      ...
      <div className="dr-panel">
        ...
        <DrLineChart id="s.recoveryLine" data={rec} color="#10B981" w={780} h={170} fill/>
...
Object.assign(window, { MarketStage, CompStage, SurvStage });
