/* DirectorAnim.jsx — 라이브 데이터 애니메이션 (플레이버 시스템)
   ─────────────────────────────────────────────────────────────
...
export const SeqCtx = createContext({ seq: {}, tab: "market", speed: 1 });

/* ───────── hooks ───────── */
export function useFx(id) {
  const { seq } = useContext(SeqCtx);
  const e = seq[id];
  return e ? { n: e.n, anim: e.anim || [] } : { n: 0, anim: [] };
}
export function useTrigger(id) { return useFx(id).n; }
export function useSpeed() {
  const { speed } = useContext(SeqCtx);
  return speed || 1;
}
...
/* ============================================================
   Sparkles
   ============================================================ */
export function Sparkles({ id, count = 6, color = "#FFE873", radius = 60 }) {
  const fx = useFx(id);
...
/* ============================================================
   DrLiveCountUp
   ============================================================ */
export function DrLiveCountUp({ id, value, duration = 1100, suffix = "", prefix = "", decimals = 0 }) {
  const fx = useFx(id);
  const speed = useSpeed();
  const target = typeof value === "string" ? parseFloat(value.replace(/,/g, "")) : value;
  const [shown, setShown] = useState(fx.n > 0 ? 0 : target);
...
/* ============================================================
   DrLineChart
   ============================================================ */
export function DrLineChart({ id, data, color = "#FFFFFF", w = 320, h = 100, label, valueLabel, fill = true, showDots = false }) {
  const fx = useFx(id);
  const speed = useSpeed();
  const ref = useRef(null);
  const [drawn, setDrawn] = useState(fx.n === 0);
...
/* ============================================================
   DrBarChart
   ============================================================ */
export function DrBarChart({ id, data, labels, color = "#FFFFFF", h = 110, label, highlightIdx = -1 }) {
  const fx = useFx(id);
  const speed = useSpeed();
...
/* ============================================================
   DrDonut
   ============================================================ */
export function DrDonut({ id, data, size = 140, label, centerLabel, centerValue }) {
  const fx = useFx(id);
  const speed = useSpeed();
  const ref = useRef(null);
...
/* ============================================================
   DrGauge
   ============================================================ */
export function DrGauge({ id, value, max = 100, label, size = 160, thresholds = [30, 60] }) {
  const fx = useFx(id);
  const speed = useSpeed();
  const [v, setV] = useState(fx.n > 0 ? 0 : value);
  ...
  const pct = v / max;
  const color = v < thresholds[0] ? "#7a7a7a"
              : v < thresholds[1] ? "#9a9a9a"
              : "#FFFFFF";
...
/* ============================================================
   DrKpiTile
   ============================================================ */
export function DrKpiTile({ id, label, value, suffix, prefix, decimals, delta, deltaPositive, tone = "blue", src, hero }) {
  const fx = useFx(id);
...
/* ============================================================
   TypingText
   ============================================================ */
export function TypingText({ text, speedMs = 22, on = true }) {
...
/* ============================================================
   DrStagger
   ============================================================ */
export function DrStagger({ id, children, delay = 80, anim = "pop", className = "", style = {} }) {
  const fx = useFx(id);
  return (
    <div
      key={`stag-${fx.n}`}
...
/* ============================================================
   DrFader
   ============================================================ */
export function DrFader({ id, children, className = "", style = {} }) {
  const fx = useFx(id);
  return (
...
/* ============================================================
   DrTween
   ============================================================ */
export function DrTween({ id, from = 0, to, duration = 1100, children }) {
  const fx = useFx(id);
  const speed = useSpeed();
  const [v, setV] = useState(fx.n > 0 ? from : to);
  useEffect(() => {
    if (fx.n === 0) return;
    let raf, t0;
    const dur = duration / speed;
