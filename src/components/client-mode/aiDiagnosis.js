...
// 매출 위치를 신호로. ★[2026-06-30 저울섞임 제거] 비교 기준(mid=시군구평균)은 이제 buildBundle에서 null로 내려온다.
//   월평균매출은 '비즈맵 저울'(동 자기 분위)인데 시군구평균은 소상공인365 구 평균이라 저울이 달라,
//   둘을 비교하면 강남이 저매출로 왜곡됐다("시군구 평균 대비 68% 낮음" = 가짜). 그래서 매출 편차 신호를
//   아예 안 만든다(mid=null → 아래 가드에서 신호 없음). 매출 서술은 동 분위(평균/상위20%/하위20%, 전부 비즈맵)로만.
...
function extractSignals(cardData, benchmarks, menuRising, season) {
  ...
  // 5 매출 — ★[2026-06-30 저울섞임 제거] 시군구평균(소상공인 구 평균)과의 비교를 폐기.
  //   월평균매출(비즈맵)과 시군구평균(소상공인)은 저울이 달라 비교하면 왜곡 → mid=null 이라 신호 안 생김.
  //   매출은 facts의 동 분위(평균/상위20%/하위20%, 전부 비즈맵)로만 서술하게 둔다.
  push(_quantileSignal('월평균매출', c5.월평균매출만원, c5.동평균매출만원, c5.시군구평균매출만원),
       5, CARD_TITLES[5]);
  // (동평균_시군구대비도 시군구평균 null 이라 자동 드롭 — 저울섞임 비교라 의도적으로 안 만든다.)
...
export function buildDiagnosisBundle({ cards, kosisBoxData, collectedData, dataAsOf, address, radius }) {
  ...
  const cardData = [
    ...
    // 5 매출 분석 (월평균=monthlyAvgSales 단일값: 비즈맵 커피전문점 점포평균 2,558 — 소상공인 1억은 폐기)
    //   ★[2026-06-30 저울섞임 제거] 월평균매출은 '비즈맵 저울'(동 자기 분위)인데, 직전엔 비교 기준 '시군구평균'을
    //     소상공인365 구 평균(guAvg=8,052)으로 줬다 → 비즈맵(2,558) vs 소상공인(8,052) = 서로 다른 저울이라
    //     "시군구 평균 대비 68% 낮음"이라는 가짜 저매출 헤드라인이 나왔다(같은 비즈맵 salesPercentile은 '좋음 65%'라 정반대).
    //   ★해법(택1-a, 정직·안전): 매출 비교 기준(시군구평균)을 통째로 null 처리한다.
    //     → _quantileSignal/_scoreCandidate 가 비교 base 없어 매출 편차 신호를 안 만들고(억지 비교 금지),
    //       프롬프트 규칙(비교기준 없으면 비교표현 금지)에 따라 "시군구 평균 대비 N%" 문장이 자동 소멸한다.
    //     매출 서술은 같은 비즈맵 저울인 동 분위(평균/상위20%/하위20%, 아래 키)로만 하게 둔다.
    //   ★[2026-06-28] 상위20%/하위20% 분위 매출은 facts 재료로 그대로 둔다(전부 비즈맵, 같은 저울).
    //     pro가 동 분위 격차를 근거로 매출을 서술 — 소상공인 구 평균과 섞지 않는다.
    _clean({
      ...
      // ★시군구평균매출만원(소상공인 guAvg)은 비교 기준에서 제거 — 비즈맵 월평균과 저울이 달라 섞으면 왜곡.
      //   매출 비교는 동 분위(상위20%/하위20%, 전부 비즈맵)로만. (값 자체는 facts sigunguAvgSales 도 안 내보냄.)
      시군구평균매출만원: null,
...
const AI_DIAG_V10_PREFIX = 'bc_ai_diag_v10pro12_';  // 종합(배너/진단/설계방향) pro 단일콜 추론 전용 캐시
//   ★[2026-06-30] v10pro11_→v10pro12_ : 매출 저울섞임 제거 — 비즈맵 월평균을 소상공인 구 평균과 비교하던 facts(salesVsSigungu/sigunguAvgSales)·신호 폐기. 옛 "시군구 평균 대비 68% 낮음" 배너/진단/설계방향 텍스트 무효화.
//   ★[2026-06-30] v10pro10_→v10pro11_ : 카페 월평균매출 단일값을 소상공인 1억→비즈맵 커피 점포평균(2,558)으로 통일. 옛 1억 기준 배너/진단/설계방향 텍스트 무효화.
...
function scoreSignals(cardData, benchmarks, menuRising, season) {
  ...
  // 5 매출 — ★[2026-06-30 저울섞임 제거] mu=시군구평균매출만원은 buildBundle에서 null로 내려온다(소상공인 구 평균).
  //   비즈맵 월평균 vs 소상공인 구 평균 = 저울이 달라 z가 왜곡 → mu null 이면 _scoreCandidate 가 z=null 로 신호 안 만듦.
  //   매출은 facts 동 분위(상위20%/하위20%, 전부 비즈맵)로만 서술한다.
  ...
  // 동평균 자체의 위치 — 시군구평균 null 이라 자동 드롭(저울섞임 비교 방지)
  take(_scoreCandidate({
...
8. 회수기간·투자 회수 개월수 등 회수 시점 예측/언급 절대 금지: "초기 투자금 회수 기간을 단축", "○개월 만에 회수", "투자금 회수" 같은 회수 시점 단정·예측을 쓰지 마라(예언성 표현). 대신 '운영 효율·비용 구조' 관점으로 서술한다.`;
...
- ★기준 라벨은 신호의 "비교기준" 값을 그대로 써라. 신호에 "비교기준"이 없으면 어떤 기준 대비로도 쓰지 마라.
- ★★[매출 저울 통일] 월평균매출(2,558 류)은 '비즈맵 동 분위' 저울이다. 절대로 "시군구 평균 대비" / "구 평균 대비" / "지역 평균 대비"로 매출을 비교하지 마라(소상공인 구 평균은 저울이 달라 섞으면 강남도 저매출로 왜곡된다 — 실제로 그래서 사고가 났다). 매출을 서술할 땐 같은 비즈맵 저울인 동 분위 facts(월평균매출·상위20% 카페 월매출·하위20% 카페 월매출·격차배수)로만 말해라. 예: "이 동 카페 월평균 2,558만원, 상위 20%는 6,998만원·하위 20%는 289만원으로 점포 간 편차가 큼". "시군구/구/지역 평균 대비 N%"라는 매출 문장은 금지.
