...
const Card4IndieCafeAnalysis = ({ card, kosisBoxData = null }) => {
  if (!card) return null;
  ...
  const priceMiniCards = [
    americanoAvg > 0 ? { label: '아메리카노 평균', value: americanoAvg, color: TOK.primary, iconKey: 'cafe' } : null,
    menuAvg > 0 ? { label: '메뉴 평균', value: menuAvg, color: TOK.secondary, iconKey: 'menu' } : null,
    dessertAvg > 0 ? { label: '디저트 평균', value: dessertAvg, color: TOK.warning, iconKey: 'cake' } : null,
  ].filter(Boolean);
  const showPriceMini = priceMiniCards.length > 0;

  // [2026-05-06] 비즈맵 메뉴 정보는 카드 3(상권 변화 추이)으로 이관됨
  const showPriceSection = showPriceMini;
  ...
  return (
    ...
    >
              ...
              {priceMiniCards.map((mc, i) => (
                <div key={i} style={{
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    {mc.iconKey === 'cafe' && (
                      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={mc.color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
      ...
      {(() => {
        ...
        return (
          <div style={{ padding: '20px 24px', borderTop: `1px solid ${TOK.borderSubtle}` }}>
            <div style={{
              ...
              <div style={{ fontSize: 13, fontWeight: 700, color: TOK.white, letterSpacing: '-0.01em' }}>
                개인 카페 폐업 신호 (국세청 {periodLabel || '작년'})
              </div>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, marginTop: 4 }}>
                <span style={{ fontSize: 32, fontWeight: 700, color: TOK.error, lineHeight: 1.1 }}>
                  {ccValue.toLocaleString('ko-KR')}
                </span>
                <span style={{ fontSize: 14, color: TOK.onSurfaceVariant }}>곳 폐업 ({region})</span>
              </div>
              {cmpLabel ? (
                <div style={{ fontSize: 12, color: cmpColor, fontWeight: cmpColor === TOK.error ? 700 : 500, marginTop: 2 }}>
                  {cmpLabel}
                </div>
              ) : (
                <div style={{ fontSize: 12, color: TOK.onSurfaceVariant, marginTop: 2 }}>
                  전국 평균 기준
                </div>
              )}
              <div style={{ fontSize: 11, color: '#999', marginTop: 6, textAlign: 'right' }}>
                국세청 100대 생활밀접업종 (커피음료점)
              </div>
            </div>
          </div>
        );
