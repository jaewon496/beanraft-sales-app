...
export function mapCollectedDataToCards(collectedData, aiData, radius = 500) {
  ...
  const card5 = {
    ...
    chartData: salesChartItems.length > 0
      ? { bigNumber: cafeSales ? Math.round(cafeSales / 10000) : (salesChartItems[salesChartItems.length - 1]?.value || 0), unit: '만원', labels: salesChartItems.map(d => d.label), values: salesChartItems.map(d => d.value) }
      : (cafeSales > 0 ? { bigNumber: Math.round(cafeSales / 10000), unit: '만원', labels: [], values: [] } : null),
    bodyData: {
      monthly: cafeSales || 0,
      dongAvg: dongAvg || 0,
      guAvg: 0,
