...
exports.handler = async (event) => {
  ...
  try {
    ...
    // 자릿수 검증: 비즈맵은 8자리(통계청) 행정동 코드 요구
    const admiCdStr = String(admiCd);
    if (admiCdStr.length !== 8) {
      console.warn(`[nicebizmap-proxy] admiCd 자릿수 이상: ${admiCd} (${admiCdStr.length}자리, 기대 8자리)`);
      // 10자리이고 끝이 "00"이면 자동 변환 시도
      if (admiCdStr.length === 10 && admiCdStr.endsWith('00')) {
        const converted = admiCdStr.slice(0, 8);
        console.log(`[nicebizmap-proxy] 10자리 → 8자리 자동 변환: ${admiCd} → ${converted}`);
        admiCd = converted;
        if (requestBody) requestBody.admiCd = converted;
      }
    }
